...
public class UserPositionController extends BaseController
{
	...
	public void save(HttpServletRequest request, HttpServletResponse response,UserPosition userPosition) throws Exception
	{
		...
		try{
			if(userPosition.getUserPosId()==null||userPosition.getUserPosId()==0){
				...
				resultMsg=getText("record.added",getText("controller.userPosition.SYS_USER_POS"));
			}else{
			    userPositionService.update(userPosition);
				resultMsg=getText("record.updated",getText("controller.userPosition.SYS_USER_POS"));
			}
	...
	public void del(HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		...
		try{
			...
			message=new ResultMessage(ResultMessage.Success, getText("record.deleted",getText("controller.userPosition.SYS_USER_POS")));
		}catch(Exception ex){
	...
	public void addPosUser(HttpServletRequest request,HttpServletResponse response) throws Exception
	{
		...
		try {
			...
			resultMessage=new ResultMessage(ResultMessage.Success,getText("controller.userPosition.addPosUser.success"));
		} catch (Exception e) {
			String str = MessageUtil.getMessage();
			if (StringUtil.isNotEmpty(str)) {
				resultMessage = new ResultMessage(ResultMessage.Fail,getText("controller.userPosition.addPosUser.fail")+":" + str);
