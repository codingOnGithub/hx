...
public class BusQueryFilterController extends BaseController {
	...
	public void save(HttpServletRequest request, HttpServletResponse response,
		...
		try {
			if (busQueryFilter.getId() == null || busQueryFilter.getId() == 0) {
				...
				resultMsg = getText("record.added", getText("controller.busQueryFilter.busQueryFilter"));
			} else {
				busQueryFilterService.update(busQueryFilter);
				resultMsg = getText("record.updated", getText("controller.busQueryFilter.busQueryFilter"));
			}
	...
	public ModelAndView list(HttpServletRequest request,
		...
		//
		url = StringUtil.getUrl(url, QueryConstants.IS_QUERY + "="
				+ QueryConstants.IS_QUERY_YES + "&"
				+ QueryConstants.FILTER_FLAG + "=");
		return this.getAutoView().addObject("myFilterList", myFilterList)
	...
	public void del(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			message = new ResultMessage(ResultMessage.Success, getText("record.deleted",getText("controller.busQueryFilter.busQueryFilter")));
		} catch (Exception ex) {
			message = new ResultMessage(ResultMessage.Fail, getText("record.delete.fail",getText("controller.busQueryFilter.busQueryFilter"))+ ex.getMessage());
		}
	...
	public Object edit(HttpServletRequest request) throws Exception {
		...
		map.put("busQueryFilter", busQueryFilter);
		return map;
	}
	...
	public Object get(HttpServletRequest request, HttpServletResponse response)
		...
		Map<String, Object> map = new HashMap<String, Object>();
		if(	busQueryFilter.getCreateBy().longValue() != ContextUtil.getCurrentUserId().longValue())
			busQueryFilter =null;
		map.put("busQueryFilter", busQueryFilter);
		return map;
	}
	...
	public void saveFilter(HttpServletRequest request,
		...
		try {
			...
			Long filterId = RequestUtil.getLong(request, "filterId");
			BusQueryFilter busQueryFilter = null;
			Boolean isAdd = true;
			if (BeanUtils.isNotEmpty(filterId) && filterId>0L ) {
				busQueryFilter = busQueryFilterService.getById(filterId);
				isAdd = false;
			} else {
				busQueryFilter = new BusQueryFilter();
			}
			...
			busQueryFilterService.saveFilter(busQueryFilter, queryMap,
					sortParameter, isAdd);
