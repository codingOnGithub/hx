...
public class BpmDefinitionController extends BaseController {
		...
		try {
			...
			if (StringUtil.isEmpty(lAryId)) {
				resultMessage = new ResultMessage(ResultMessage.Success,
						getText("controller.bpmDefinition.del.notDefId"));
			} else {
				String[] aryDefId = lAryId.split(",");
				for (String defId : aryDefId) {
					Long lDefId = Long.parseLong(defId);
					//对于已发布的流程，需要删除其国际化资源
					BpmDefinition bpmDefinition=bpmDefinitionService.getById(lDefId);
					if (BeanUtils.isNotEmpty(bpmDefinition)&&bpmDefinition.getStatus()!=0) {
						bpmFormLanguageService.delByFormIdAndType(bpmDefinition.getActDefId(), BpmFormLanguage.BPM_DEFINITION_TYPE);
					}
					// 级联删除流程定义
					bpmDefinitionService.delDefbyDeployId(lDefId, onlyVersion);
				}
				resultMessage = new ResultMessage(ResultMessage.Success,
						getText("record.deleted",getText("bpmDefinition.export.bpmDefinition")));
			}
		} catch (Exception e) {
			e.printStackTrace();
			resultMessage = new ResultMessage(ResultMessage.Fail, getText("record.delete.fail",getText("bpmDefinition.export.bpmDefinition"))+":"
					+ e.getMessage());
		}
	...
	public Map<String, Object> getUserSetByDefNodeId(HttpServletRequest request, HttpServletResponse response) throws Exception {
		Long defId = RequestUtil.getLong(request, "defId");
		String nodeId = RequestUtil.getString(request, "nodeId");
		// 单节点人员设置
		BpmNodeSet nodeSet = bpmNodeSetService.getByDefIdNodeId(defId,nodeId);
	...
	public ModelAndView design(HttpServletRequest request,
		...
		String localeLang=ContextUtil.getLocale().toString();
		// xml流程分类
	...
	@Action(description = "流程在线设计，保存，发布操作",execOrder=ActionExecOrder.AFTER,
	...
	public ModelAndView flexDefSave(HttpServletRequest request,
		...
		try {
			...
			//添加流程定义配置。
			SysAuditThreadLocalHolder.putParamerter("bpmDefinition", bpmDefinition);
			response.getWriter().print("success");
		} catch (Exception ex) {
			ex.printStackTrace();
			String str = MessageUtil.getMessage();
			if (StringUtil.isNotEmpty(str)) {
				resultMessage = new ResultMessage(ResultMessage.Fail,getText("controller.bpmDefinition.saveFail") + str);
	...
	@Action(description = "删除流程节点的人员设置",
			execOrder=ActionExecOrder.BEFORE,
			detail = "<#assign nodeuser=bpmNodeUserService.getById(Long.valueOf(nodeUserId))/>" +
	...
	public void saveCondition(HttpServletRequest request,
		...
		try {
			...
			writeResultMessage(writer, getText("controller.save.fail")+":" + e.getMessage(),
					ResultMessage.Fail);
		}
	}
...
//		else name="多条流程定义记录_"+nowDate;
		...
		try {
			...
			if (StringUtil.isNotEmpty(str)) {
				message = new ResultMessage(
						ResultMessage.Fail,getText("controller.bpmDefinition.importFail") + str);
			} else {
	...
	public void saveForkJoin(HttpServletRequest request,
		...
		if (joinKeyNodeSet != null) {
			if (bpmNodeSet == null
				...
				writeResultMessage(
						response.getWriter(),
						getText("controller.bpmDefinition.saveForkJoin.message",joinTaskName,joinKeyNodeSet.getNodeName()),
						ResultMessage.Fail);
				return;
			}
		}
		if (bpmNodeSet != null) {
			...
			writeResultMessage(response.getWriter(), getText("controller.bpmDefinition.saveForkJoin.success"),
					ResultMessage.Success);
		}
	}
	...
	public void cleanData(HttpServletRequest request,HttpServletResponse response) throws Exception{
		...
		try {
			...
			writeResultMessage(response.getWriter(), new ResultMessage(
					ResultMessage.Success, getText("controller.bpmDefinition.cleanData.success")));
		} catch (Exception e) {
			e.printStackTrace();
			writeResultMessage(response.getWriter(), new ResultMessage(
					ResultMessage.Fail, getText("controller.bpmDefinition.cleanData.fail")));
		}
	...
	public ModelAndView informType(HttpServletRequest request,HttpServletResponse response)throws Exception
	{
		...
		return getAutoView().addObject("bpmNodeSet", bpmNodeSet).addObject("handlersMap",handlersMap);
	}
	...
	public void saveInformType(HttpServletRequest request,HttpServletResponse response)throws Exception
	{
		...
		try {
			bpmNodeSetService.update(bpmNodeSet);
			writeResultMessage(response.getWriter(), new ResultMessage(
					ResultMessage.Success, getText("controller.setDefault.success")));
		} catch (Exception e) {
			String str = MessageUtil.getMessage();
			if (StringUtil.isNotEmpty(str)) {
				ResultMessage resultMessage = new ResultMessage(
						ResultMessage.Fail, getText("controller.setDefault.fail")+":" + str);
				response.getWriter().print(resultMessage);
			} else {
	...
	public ModelAndView conditionEdit(HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		if(bpmFormResult.getResult()==1){
			return ServiceUtil.getTipInfo(getText("controller.bpmDefinition.conditionEdit.message"));
		}
	...
	public ModelAndView nodeSummary(HttpServletRequest request,
		...
		String actDefId = bpmDefinition.getActDefId();
		//获取国际化资源
		Map<String,String> languageMap = bpmFormLanguageService.getLanguageMapByActDefId(actDefId,BpmFormLanguage.BPM_DEFINITION_TYPE);
		...
		startFlowNode = getFlowNodeLanguage(startFlowNode,languageMap);
		List<FlowNode> endFlowNodeList = new ArrayList<FlowNode>();
		List<FlowNode> endFlowNodes = NodeCache.getEndNode(actDefId);
		for (FlowNode endFlowNode : endFlowNodes) {
			endFlowNode = getFlowNodeLanguage(endFlowNode,languageMap);
			endFlowNodeList.add(endFlowNode);
		}
	...
	private FlowNode getFlowNodeLanguage(FlowNode flowNode,
	...
	private BpmDefinition getBpmDefinitionLanguage(BpmDefinition bpmDefinition, Map<String, String> languageMap) {
	...
	public String checkVersion(HttpServletRequest request,
		...
		try {
			...
			jsonObject.accumulate("success", true)
					...
					.accumulate("msg", getText("controller.bpmDefinition.checkVersion.message1"));
		} catch (Exception e) {
			jsonObject.accumulate("success", false).accumulate("msg",
	...
	public void saveReferDef(HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		writeResultMessage(response.getWriter(), getText("controller.bpmDefinition.saveReferDef"),
				ResultMessage.Success);
	...
	public void delReferDef(HttpServletRequest request,
		...
		try {
			...
			writeResultMessage(response.getWriter(),  getText("controller.del.success"),
					ResultMessage.Success);
		} catch (Exception e) {
			writeResultMessage(response.getWriter(),  getText("controller.del.fail"),
					ResultMessage.Success);
		}
	}
