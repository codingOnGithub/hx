...
public class SysOfficeTemplateController extends BaseController
{
	...
	public void del(HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		...
		try{
			...
			if(!saveType.contains("database")){              //保存在服务器时才做文件删除
				...
				//	String filePath = sysOfficeTemplate.getPath();
	...
	public ModelAndView get(HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		long id=RequestUtil.getLong(request,"id");
		SysOfficeTemplate sysOfficeTemplate = sysOfficeTemplateService.getById(id);		
		return getAutoView().addObject("sysOfficeTemplate", sysOfficeTemplate).addObject("saveType", saveType);
	}
		...
		try{
			...
			if(it.hasNext()) {
			    ...
			    if(isOfficeFile(extName)==false){
			    	writeResultMessage(response.getWriter(),new ResultMessage(ResultMessage.Fail,getText("controller.sysOfficeTemplate.fileValidate")+"："+doc));
			    	return;
			    }
			  ...
			  //  String basePath = ServiceUtil.getBasePath();
			    ...
			    if(id==0){
			    	...
			    	resultMsg=getText("record.added",getText("controller.sysOfficeTemplate"));
			    }else{
			    	sysOfficeTemplate.setId(id);
			    	sysOfficeTemplateService.update(sysOfficeTemplate);
			    	resultMsg=getText("record.updated",getText("controller.sysOfficeTemplate"));
			    }
			...
			if (StringUtil.isNotEmpty(str)) {
				ResultMessage resultMessage = new ResultMessage(ResultMessage.Fail,getText("controller.sysOfficeTemplate.operate.fail")+":" + str);
