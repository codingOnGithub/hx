...
public class MainController extends BaseController {
	...
	public ModelAndView main(HttpServletRequest request,
		...
		//语言
		List<SysLanguage> sysLanguages = sysLanguageService.getUserableLangauge();
		...
		if(currentSystem != null){
			...
			if(StringUtils.isNotEmpty(logo)&&!logo.contains(skinStyle)){//如果不是当前样式则替换
				String style = "/styles/";
				Integer startIndex = logo.indexOf(style);
				Integer endIndex = logo.indexOf("/images/");
				String oldStyle = logo.substring(startIndex + style.length(), endIndex);
				logo = logo.replace(oldStyle, skinStyle);
			}
		...
		return this.getView("console",mainViewName)
			...
			.addObject("sysLanguages", sysLanguages)
	...
	public ModelAndView home(HttpServletRequest request,
		...
		Map<?, ?> mapData = desktopMycolumnService.getMyDeskData(userId,
				ctxPath);
		if (mapData == null)
			return null;
	...
	public List<Resources> getSysRolResTreeData(HttpServletRequest request,
		...
		SubSystem currentSystem = SubSystemUtil.getCurrentSystem(request);
		List<Resources> resourcesList = resourcesService.getSysMenu(
				currentSystem, ContextUtil.getCurrentUser(), lanType);

		ResourcesService
				.addIconCtxPath(resourcesList,request);
