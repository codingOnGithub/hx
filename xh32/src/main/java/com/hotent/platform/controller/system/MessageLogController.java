	...
	public void save(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			if (messageLog.getId() == null || messageLog.getId() == 0) {
				...
				resultMsg = getText("record.added",getText("controller.messageLog"));
			} else {
				messageLogService.update(messageLog);
				resultMsg = getText("record.updated",getText("controller.messageLog"));
			}
			//写日志时需要的参数
			...
			if (StringUtil.isNotEmpty(str)) {
				ResultMessage resultMessage = new ResultMessage(ResultMessage.Fail,getText("controller.messageLog.saveOrUpdFail")+":" + str);
