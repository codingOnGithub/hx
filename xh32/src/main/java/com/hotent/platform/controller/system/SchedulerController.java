...
public class SchedulerController extends BaseController{
			...
			try
				{
					...
					if(isExist)
					{
						ResultMessage obj=new ResultMessage(ResultMessage.Fail,getText("controller.scheduler.jobName.already"));
						out.print(obj.toString());
					}
					else
					{
						schedulerService.addJob(jobName, className, parameterJson,description);
						ResultMessage obj=new ResultMessage(ResultMessage.Success,getText("record.added", getText("controller.scheduler")));
						out.print(obj.toString());
					}
				}
				catch(ClassNotFoundException ex)
				{
					ResultMessage obj=new ResultMessage(ResultMessage.Fail,getText("controller.scheduler.jobClass.notExisted"));
					out.print(obj.toString());
				...
				catch (Exception ex)
				{
					...
					if (StringUtil.isNotEmpty(str)) {
						ResultMessage resultMessage = new ResultMessage(ResultMessage.Fail,getText("controller.add.fail")+":" + str);
	...
	public ModelAndView addTrigger(HttpServletResponse response, HttpServletRequest request, @PathVariable("viewName") String viewName) throws IOException, SchedulerException, ParseException 
	{
		...
		else if(STEP2.equals(viewName))
		{
			...
			if(rtn)
			{
				ResultMessage obj=new ResultMessage(ResultMessage.Fail,getText("controller.scheduler.planName.already"));
				out.print(obj.toString());
			}
			try {
				schedulerService.addTrigger(jobName, trigName, planJson);
				ResultMessage obj=new ResultMessage(ResultMessage.Success,getText("record.added",getText("controller.scheduler.plan")));
				out.print(obj.toString());
			} catch (Exception e) {
				String str = MessageUtil.getMessage();
				if (StringUtil.isNotEmpty(str)) {
					ResultMessage resultMessage = new ResultMessage(ResultMessage.Fail,getText("controller.del.fail")+":" + str);
					response.getWriter().print(resultMessage);
				} else {
	...
	public void validClass(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (rtn) {
			ResultMessage obj = new ResultMessage(ResultMessage.Success, getText("controller.scheduler.validClass.success"));
			out.println(obj.toString());
		} else {
			ResultMessage obj = new ResultMessage(ResultMessage.Fail, getText("controller.scheduler.validClass.fail"));
			out.println(obj.toString());
		}
	}
	/**
	...
	public void delTrigger(HttpServletResponse response, HttpServletRequest request) throws IOException, SchedulerException, ClassNotFoundException 
	{
		...
		try {
			...
			message=new ResultMessage(ResultMessage.Success,getText("record.deleted",getText("controller.scheduler.plan")));
		} catch (Exception e) {
			message=new ResultMessage(ResultMessage.Fail,getText("controller.del.fail")+":"+e.getMessage());
		}
		addMessage(message, request);
	...
	public void del(HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		...
		try {
			...
			message=new ResultMessage(ResultMessage.Success,getText("record.deleted",getText("controller.scheduler.jobLog")));
		} catch (Exception e) {
			message=new ResultMessage(ResultMessage.Fail,getText("controller.del.fail")+":"+e.getMessage());
		}
		addMessage(message, request);
