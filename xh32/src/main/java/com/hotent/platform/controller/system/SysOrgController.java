...
public class SysOrgController extends BaseController {
	...
	public void orgdel(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			delByIds(lAryId);
			message = new ResultMessage(ResultMessage.Success, getText("record.deleted",getText("controller.sysOrg.OrgAndSubOrg")));
		} catch (Exception e) {
			message = new ResultMessage(ResultMessage.Fail, getText("record.delete.fail",getText("controller.sysOrg.OrgAndSubOrg")));
		}
	 ...
	 * 组织删除，非物理删除
	 * @author hjx
	 * @version 创建时间：2013-12-13  上午9:51:03
	 * @param lAryId
	 */
	...
	public List<SysOrg> getTreeOnlineData(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		orgMap.put(0L, getRootSysOrg(-1L, getText("controller.sysOrg.notDisOrg")));// 添加一个未分配组织的项
		...
		if (!StringUtils.isEmpty(depTreeRootId)) {
			SysOrg org = getRootSysOrg(demId, getText("controller.sysOrg.All"));
			if (demension != null)
	...
	public void move(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			resultMessage = new ResultMessage(ResultMessage.Success, getText("controller.globalType.move.success"));
		} catch (Exception ex) {
			String str = MessageUtil.getMessage();
			if (StringUtil.isNotEmpty(str)) {
				resultMessage = new ResultMessage(ResultMessage.Fail,getText("controller.globalType.move.fail")+":" + str);
	...
	public void sort(HttpServletRequest request,HttpServletResponse response) throws Exception{
		...
		resultObj=new ResultMessage(ResultMessage.Success,getText("controller.globalType.sort.success"));
		out.print(resultObj);	
	}
	/**
	 * 获取参数列表mv。
