...
import com.hotent.platform.model.form.FormLanguage;
...
public class BpmFormDefController extends BaseController {
	...
	public void newVersion(HttpServletRequest request,
		...
		try {
			...
			msg = new ResultMessage(ResultMessage.Success, getText("controller.bpmFormDef.newVersion.success"));
		} catch (Exception ex) {
			msg = new ResultMessage(ResultMessage.Fail, getText("controller.bpmFormDef.newVersion.success"));
		}
...
/*	@ResponseBody
		...
		for(Long formKey:aryFormKey){
			...
			if (rtn > 0) {
				...
				try {
					...
					msg = new ResultMessage(ResultMessage.Fail, getText("controller.del.fail"));
				}
			}
		}
		if (error) {
			if(subject.size()>1){
				...
				msg = new ResultMessage(ResultMessage.Fail, getText("controller.bpmFormDef.delByFormKey.relation")+result);
			...
			msg = new ResultMessage(ResultMessage.Success,  getText("controller.del.success" ));
		}
	...
	public void getHtmlByAlias(HttpServletRequest request,
		...
		PrintWriter out = response.getWriter();
		String templatePath = FormUtil.getDesignTemplatePath();
		String html = FileUtil.readFile(templatePath + tempalias + ".html");
		String reult=this.filedInternation(html);
		out.println(reult);
	}
	...
	public void publish(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			service.publish(formDefId, ContextUtil.getCurrentUser()
					.getFullname());
			resultObj = new ResultMessage(ResultMessage.Success, getText("controller.bpmFormDef.publish"));
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void saveForm(HttpServletRequest request,
		...
		message = (publish == 1) ?  getText("controller.bpmFormDef.saveForm.publish") :getText("controller.bpmFormDef.saveForm.success");
		...
		try {
			...
			String str = MessageUtil.getMessage();
			if (StringUtil.isNotEmpty(str)) {
				resultMessage = new ResultMessage(ResultMessage.Fail, getText("controller.bpmFormDef.saveForm.fail")+":"
						+ str);
			} else {
