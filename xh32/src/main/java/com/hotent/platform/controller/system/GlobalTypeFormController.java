...
public class GlobalTypeFormController extends BaseFormController
{
	...
	public void save(HttpServletRequest request, HttpServletResponse response, GlobalType globalType,BindingResult bindResult) throws Exception
	{
		...
		JSONArray array = JSONArray.fromObject(typeNames);
		...
		if(globalType.getTypeId()==0){
			...
			if(StringUtil.isNotEmpty(nodeKey)){
				...
				if(isExist){
					resultMsg=getText("controller.globalType.keyAlready");
			...
			if(null!=array && array.size()>0)
			{
			    for(int p=0;p<array.size();p++)
			    {
					...
					globalTypeService.addLanGlobalType(globalType.getTypeId().toString(), catKey, 
							obj.get("resValue").toString(), obj.get("lanType").toString());
			    }
			}
			
			resultMsg=getText("record.added",getText("controller.globalType"));
		}else{
			...
			if(null!=array && array.size()>0)
			{
			    ...
			    {
			    	JSONObject obj = array.getJSONObject(p);
					globalTypeService.updateLanGlobalType(typeId.toString(), catKey, 
							obj.get("resValue").toString(), obj.get("lanType").toString());
			    }
			}
			resultMsg=getText("record.updated",getText("controller.globalType"));
		}
