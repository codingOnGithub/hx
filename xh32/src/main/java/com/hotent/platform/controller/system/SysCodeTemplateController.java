...
public class SysCodeTemplateController extends BaseController
{
	...
	public void save(HttpServletRequest request, HttpServletResponse response,SysCodeTemplate sysCodeTemplate) throws Exception
	{
		...
		try{
			...
			if(sysCodeTemplate.getId()==null){
				...
				resultMsg=getText("record.added",getText("controller.sysCodeTemplate"));
			}else{
			    sysCodeTemplateService.update(sysCodeTemplate);
				resultMsg=getText("record.updated",getText("controller.sysCodeTemplate"));
				isadd=false;
			}
	...
	{	
		...
		try{
			...
			message=new ResultMessage(ResultMessage.Success, getText("controller.sysCodeTemplate.init.success"));
		}catch(Exception ex){
			message=new ResultMessage(ResultMessage.Fail, getText("controller.sysCodeTemplate.init.fail") + ":" + ex.getMessage());
		}
		addMessage(message, request);
		response.sendRedirect(preUrl);
	}
