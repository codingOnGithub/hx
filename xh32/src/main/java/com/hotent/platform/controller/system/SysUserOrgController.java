...
//	}
	...
	public void setIsPrimary(HttpServletRequest request, HttpServletResponse response) throws IOException
	{
		...
		try{
			...
			message=new ResultMessage(ResultMessage.Success, getText("record.setted",getText("controller.sysUserOrg.org")));
		}
		catch(Exception ex){
			issuccess=false;
			message=new ResultMessage(ResultMessage.Fail, getText("record.set.fail",getText("controller.sysUserOrg.org")));
		}
		SysAuditThreadLocalHolder.putParamerter("isSuccess", issuccess);
	...
	@Action(description="设置是负责人",execOrder=ActionExecOrder.AFTER,
	detail="<#assign entity=SysAuditLinkService.getByUserPosId(Long.valueOf(userPosId))/>" +
	...
	public void setIsCharge(HttpServletRequest request, HttpServletResponse response) throws IOException
	{
		...
		try{
			...
			message=new ResultMessage(ResultMessage.Success, getText("record.setted",getText("controller.sysUserOrg.director")));
		}
		catch(Exception ex){
			message=new ResultMessage(ResultMessage.Fail, getText("record.set.fail",getText("controller.sysUserOrg.director")));
			issuccess=false;
		}
		SysAuditThreadLocalHolder.putParamerter("isSuccess", issuccess);
		addMessage(message, request);
	...
	public void setIsManage(HttpServletRequest request, HttpServletResponse response) throws IOException{
		...
		try{
			...
			message=new ResultMessage(ResultMessage.Success, getText("record.setted",getText("controller.sysUserOrg.admin")));
		}
		catch(Exception ex){
			message=new ResultMessage(ResultMessage.Fail, getText("record.set.fail",getText("controller.sysUserOrg.admin")));
			issuccess=false;
		}
		SysAuditThreadLocalHolder.putParamerter("isSuccess", issuccess);
		addMessage(message, request);
