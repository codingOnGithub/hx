	...
	public void save(HttpServletRequest request, HttpServletResponse response, SubSystem subSystem, BindingResult bindResult) throws Exception {
		...
		try {
			if (subSystem.getSystemId() == 0L) {
				...
				if (rtn > 0) {
					writeResultMessage(response.getWriter(), getText("controller.subsystem.alias.existed"), ResultMessage.Fail);
					return;
				}
				...
				resultMsg = getText("record.added",getText("controller.subsystem"));
				...
				resultMsg = getText("record.updated",getText("controller.subsystem"));
				writeResultMessage(response.getWriter(), resultMsg, ResultMessage.Success);
			}
			...
			if (StringUtil.isNotEmpty(str)) {
				resultMessage = new ResultMessage(ResultMessage.Fail,getText("controller.subsystem.operate.fail") + ":" + str);
