...
public class SysGradeManageController extends BaseController{
	...
	private void addOrgRole(HttpServletRequest request, HttpServletResponse response,int grade) throws IOException{
		...
		if(StringUtil.isEmpty(roleIds)){
			writeResultMessage(response.getWriter(), getText("controller.sysGradeManage.noChooseRole"), ResultMessage.Fail);
			return ;
		}
		try{
			boolean rtn= sysOrgRoleManageService.addOrgRole(orgId, roleIds,grade);
			if(rtn){
				String msg=getText("record.added",getText("controller.sysGradeManage.role"));
				writeResultMessage(response.getWriter(), msg, ResultMessage.Success);
			}
			else{
				String msg=getText("controller.sysGradeManage.isExist");
			...
			if (StringUtil.isNotEmpty(str)) {
				ResultMessage resultMessage = new ResultMessage(ResultMessage.Fail,getText("record.add.fail",getText("controller.sysGradeManage.role"))+":" + str);
	...
	private void delete(HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		...
		try{
			...
			message=new ResultMessage(ResultMessage.Success, getText("controller.sysGradeManage.delSuccess"));
		}catch(Exception ex){
