...
public class SysOrgTypeController extends BaseController
{
	...
	public void save(HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		...
		try{
			if(sysOrgType.getId()==null||sysOrgType.getId()==0){
				...
				resultMsg=getText("record.saved",getText("controller.sysOrgType"));
			}else{
			    sysOrgTypeService.update(sysOrgType);
				resultMsg=getText("record.updated",getText("controller.sysOrgType"));
			}
			writeResultMessage(response.getWriter(),resultMsg,ResultMessage.Success);
		}catch(Exception e){
			String str = MessageUtil.getMessage();
			if (StringUtil.isNotEmpty(str)) {
				ResultMessage resultMessage = new ResultMessage(ResultMessage.Fail,getText("controller.sysOrgType.addOrUpdFail")+":" + str);
	...
	public void del(HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		...
		try{
			...
			message=new ResultMessage(ResultMessage.Success, getText("record.deleted",getText("controller.sysOrgType")));
		}catch(Exception ex){
	...
	public void multSave(HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		...
		try{
			...
			String str = MessageUtil.getMessage();
			if (StringUtil.isNotEmpty(str)) {
				ResultMessage resultMessage = new ResultMessage(ResultMessage.Fail,getText("record.save.fail",getText("controller.sysOrgType"))+":" + str);
				response.getWriter().print(resultMessage);
			} else {
				String message = ExceptionUtil.getExceptionMessage(e);
				ResultMessage resultMessage = new ResultMessage(ResultMessage.Fail, message);
