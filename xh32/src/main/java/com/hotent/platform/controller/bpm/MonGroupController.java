...
public class MonGroupController extends BaseController
{
	...
	public void save(HttpServletRequest request, HttpServletResponse response,MonGroup group) throws Exception
	{
		...
		try{
			if(monGroup.getId()==null||monGroup.getId()==0){
				...
				resultMsg=getText("record.added",getText("controller.monGrou"));
			}else{
			    monGroupService.updateAll(monGroup);
				resultMsg=getText("record.updated",getText("controller.monGrou"));
			}
	...
	public void del(HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		...
		try{
			...
			message=new ResultMessage(ResultMessage.Success,getText("record.deleted",getText("controller.monGrou")));
		}catch(Exception ex){
			message=new ResultMessage(ResultMessage.Fail, getText("record.delete.fail",getText("controller.monGrou")) + ex.getMessage());
		}
	...
	public void saveAuth(HttpServletRequest request, HttpServletResponse response) throws Exception{
		...
		if(StringUtil.isEmpty(orgIds) ){
			resultMessage=new ResultMessage(ResultMessage.Fail, getText("controller.monGrou.saveAuth.notOrg"));
		}
		else if(StringUtil.isEmpty(roleIds)){
			resultMessage=new ResultMessage(ResultMessage.Fail, getText("controller.monGrou.saveAuth.notRole"));
		}
		else{
			try{
				monOrgRoleService.saveAuth(groupId, roleIds, orgIds);
				resultMessage=new ResultMessage(ResultMessage.Success, getText("controller.monGrou.saveAuth.success"));
			}
			catch (Exception e) {
				e.printStackTrace();
