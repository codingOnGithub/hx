		...
		try {
			...
			if (table.getTableId() == 0) {
				if (bpmFormTableService.isTableNameExisted(table.getTableName())) {
					msg = getText("controller.bpmFormTable.tableName");
				...
				if (rtn == -1) {
					...
					msg = getText("controller.bpmFormTable.reservedField",args);
					writeResultMessage(response.getWriter(), msg,ResultMessage.Fail);
					return;
				}
				isadd=1;
				msg = getText("record.added", getText("controller.bpmFormTable.table"));
			} else {
				boolean isExist = bpmFormTableService.isTableNameExistedForUpd(table.getTableId(), table.getTableName());
				if (isExist) {
					//"输入的表名在系统中已经存在!"
					msg = getText("controller.bpmFormTable.already");
					writeResultMessage(response.getWriter(), msg,ResultMessage.Fail);
					return;
				}
				...
				if (rtn == -1) {
					...
					msg = getText("controller.bpmFormTable.hashData");
					writeResultMessage(response.getWriter(), msg,ResultMessage.Fail);
					return;
				} else if (rtn == 0) {
					msg = getText("record.updated", getText("controller.bpmFormTable.table"));
				}
			}
			// 是否需要为已生成的主表 生成新的子表
			if (generator == 1) {
	...
	public void saveExtTable(HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		try {
			if (table.getTableId() == 0) {
				...
				msg = getText("controller.bpmFormTable.saveExtTable.success");
				writeResultMessage(response.getWriter(), msg,ResultMessage.Success);
			}
			else {
			...
			String str = MessageUtil.getMessage();
			if (StringUtil.isNotEmpty(str)) {
				ResultMessage resultMessage = new ResultMessage(ResultMessage.Fail, getText("controller.bpmFormTable.saveExtTable.fail") + ":" + str);
				response.getWriter().print(resultMessage);
			} else {
				String message = ExceptionUtil.getExceptionMessage(ex);
				ResultMessage resultMessage = new ResultMessage(ResultMessage.Fail, message);
