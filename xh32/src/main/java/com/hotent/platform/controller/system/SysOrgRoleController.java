...
public class SysOrgRoleController extends BaseController
{
	...
	public void save(HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		...
		try{
			if(sysOrgRole.getId()==null||sysOrgRole.getId()==0){
				...
				resultMsg=getText("record.added",getText("controller.sysOrgRole.OrgRoleGrantInfor"));
			}else{
			    sysOrgRoleService.update(sysOrgRole);
				resultMsg=getText("record.updated",getText("controller.sysOrgRole.OrgRoleGrantInfor"));
			}
			writeResultMessage(response.getWriter(),resultMsg,ResultMessage.Success);
		}catch(Exception e){
			String str = MessageUtil.getMessage();
			if (StringUtil.isNotEmpty(str)) {
				ResultMessage resultMessage = new ResultMessage(ResultMessage.Fail,getText("controller.sysOrgRole.OrgRoleGrantInFail")+":" + str);
	...
	private void addOrgRole(HttpServletRequest request,HttpServletResponse response,int grade) throws Exception{
		...
		try {
			...
			resultMessage=new ResultMessage(ResultMessage.Success,getText("controller.sysOrgRole.grantSuccess")+result);
			writeResultMessage(response.getWriter(), resultMessage);
		} catch (Exception e) {
			String str = MessageUtil.getMessage();
			if (StringUtil.isNotEmpty(str)) {
				resultMessage = new ResultMessage(ResultMessage.Fail,getText("controller.sysOrgRole.grantFail")+":" + str);
				response.getWriter().print(resultMessage);
			} else {
				String message = ExceptionUtil.getExceptionMessage(e);
	...
	private void delete(HttpServletRequest request, HttpServletResponse response) throws Exception{
		...
		try{
			...
			message=new ResultMessage(ResultMessage.Success, getText("record.deleted",getText("controller.sysOrgRole.OrgRoleGrantInfor")));
		}catch(Exception ex){
