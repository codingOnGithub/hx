...
public class TaskController extends BaseController {
	...
	public ModelAndView startFlowForm(HttpServletRequest request,
		...
		if (BeanUtils.isEmpty(bpmDefinition))
				return ServiceUtil.getTipInfo(getText("controller.task.startFlowForm.del"));	
		if (bpmDefinition.getStatus().shortValue() ==BpmDefinition.STATUS_DISABLED.shortValue()
			...
			return ServiceUtil.getTipInfo(getText("controller.task.startFlowForm.disable"));
		...
		try {
			...
			if(runId!=0L){
				...
				if(BeanUtils.isEmpty(processRun)){
					out.print(new ResultMessage(ResultMessage.Fail,getText("controller.task.startFlow.draft.deleteOrNotExist")));
					return;
				}
			...
			ResultMessage resultMessage = new ResultMessage(ResultMessage.Success, getText("controller.task.startFlow.success"));
			...
			if (StringUtil.isNotEmpty(str)) {
				ResultMessage resultMessage = new ResultMessage(ResultMessage.Fail, getText("controller.task.startFlow.fail")+":\r\n" + str);
				out.print(resultMessage);
			} else {
	...
	public void saveData(HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		try {
			...
			resultMessage = new ResultMessage(ResultMessage.Success,getText("controller.task.startFlow.saveFormData.success"));
		} catch (Exception ex) { 
			String str = MessageUtil.getMessage();
			if (StringUtil.isNotEmpty(str)) {
	...
	public String saveOpinion(HttpServletRequest request,HttpServletResponse response, TaskOpinion taskOpinion)throws Exception {
		...
		try {
			...
			String memo = getText("controller.task.saveOpinion.memo",processRun.getSubject(),taskEntity.getName(),taskOpinion.getOpinion());
			bpmRunLogService.addRunLog(runId,
					BpmRunLog.OPERATOR_TYPE_ADDOPINION, memo);

			jobject.accumulate("result", ResultMessage.Success)
			   .accumulate("message", getText("controller.task.saveOpinion.addOpin.success"));
			return jobject.toString();
		} catch (Exception ex) {
			String str = MessageUtil.getMessage();
			if (StringUtil.isNotEmpty(str)) {
				jobject.accumulate("result", ResultMessage.Fail)
				   .accumulate("message", getText("controller.task.saveOpinion.addOpin.fail")+":"+ str);
				return jobject.toString();
			} else {
				String message = ExceptionUtil.getExceptionMessage(ex);
				jobject.accumulate("result", ResultMessage.Fail)
	...
	private ModelAndView getToStartView(HttpServletRequest request,HttpServletResponse response,ModelAndView mv,int isManage) throws Exception{
		...
		if(StringUtil.isEmpty(taskId) && StringUtil.isEmpty(instanceId)){
			return ServiceUtil.getTipInfo(getText("controller.task.toStart.notTaskId"));
		}
		...
		if (taskEntity == null) {
			...
			if(taskHistory==null){
				if(StringUtil.isEmpty(taskId) && StringUtil.isEmpty(instanceId)){
					return ServiceUtil.getTipInfo(getText("controller.task.taskIdError"));
				}
			}
			...
			if(StringUtils.isEmpty(actInstId) && taskHistory.getDescription().equals(TaskOpinion.STATUS_COMMUNICATION.toString())){
				return ServiceUtil.getTipInfo(getText("controller.task.taskHandled"));
			}
		...
		if(TaskOpinion.STATUS_TRANSTO_ING.toString().equals(taskEntity.getDescription()) && 
			...
			return ServiceUtil.getTipInfo(getText("controller.task.taskcircuing"));
		}
		...
		if(isManage==0){
			...
			if(!hasRights){
				return ServiceUtil.getTipInfo(getText("controller.task.notExecutors"));
			}
		}
		//更新任务为已读。
		...
		if (bpmDefinition.getIsUseOutForm() == 1) {
			...
			if(StringUtils.isEmpty(formUrl)){			
				ModelAndView rtnModel=ServiceUtil.getTipInfo(getText("controller.task.setFormUrl"));
				return rtnModel;
			}
		...
		if(!formModel.isValid()){
			ModelAndView rtnModel=ServiceUtil.getTipInfo(getText("controller.task.formIsChange"));
			return rtnModel;
		}
		...
		public void toStartCommunication(HttpServletRequest request,HttpServletResponse response) throws Exception{	
			...
			if(StringUtil.isEmpty(cmpIds)){
				resultMessage=new ResultMessage(ResultMessage.Fail, getText("controller.task.toStartComm.inputCommunicaer"));
				out.print(resultMessage);
				return;
			} 
			try{
				...
				String memo=getText("controller.task.saveOpinion.memo",processRun.getSubject(),taskEntity.getName(), opinion);
	...
	public void complete(HttpServletRequest request,
		...
		if (ServiceUtil.isAssigneeNotEmpty( assignee ) && !task.getAssignee().equals(userId.toString())&& !isAdmin  ) {
			...
			try {
				...
				resultMessage = new ResultMessage(ResultMessage.Fail,getText("controller.task.complete.varIsExist")+":" + ex.getMessage() );
				//添加错误消息到日志
...
//	}
	...
	public void end(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			ProcessRun processRun = processRunService
					.getByActInstanceId(new Long(taskEntity.getProcessInstanceId()));
			String voteContent = getText("controller.task.end.compile",ContextUtil.getCurrentUser().getFullname());
			...
			String str = MessageUtil.getMessage();
			if (StringUtil.isNotEmpty(str)) {
				resultMessage = new ResultMessage(ResultMessage.Fail, getText("controller.task.end.fail")+":"
						+ str);
			} else {
				String message = ExceptionUtil.getExceptionMessage(ex);
				resultMessage = new ResultMessage(ResultMessage.Fail, message);
