...
public class MonOrgRoleController extends BaseController
{
	...
	public void save(HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		...
		try{
			if(monOrgRole.getId()==null||monOrgRole.getId()==0){
				...
				resultMsg=getText("record.added",getText("controller.monOrgRole"));
			}else{
			   // monOrgRoleService.update(monOrgRole);
				resultMsg=getText("record.updated",getText("controller.monOrgRole"));
			}
	...
	public void del(HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		...
		try{
			...
			message=new ResultMessage(ResultMessage.Success, getText("record.deleted",getText("controller.monOrgRole")));
		}catch(Exception ex){
			message=new ResultMessage(ResultMessage.Fail,getText("record.delete.fail",getText("controller.monOrgRole")) + ex.getMessage());
		}
