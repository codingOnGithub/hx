...
public class DesktopColumnController extends BaseController
{
	...
	{	
		...
		try{
			...
			message=new ResultMessage(ResultMessage.Success, getText("controller.desktopColumn.initTemplate.success"));
		}
		catch(Exception ex){
			message=new ResultMessage(ResultMessage.Fail, getText("controller.desktopColumn.initTemplate.fail")+":" + ex.getMessage());
		}
		addMessage(message, request);
		response.sendRedirect(preUrl);
	}
