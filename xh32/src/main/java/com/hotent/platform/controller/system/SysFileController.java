...
public class SysFileController extends BaseController {
	...
	//	long userId = ContextUtil.getCurrentUserId(); // 获取当前用户的id
		...
		try{			
			Map<String, MultipartFile> files = request.getFileMap();
			Map map = new HashMap();
			String reutrnStr = this.save(files, typeId, fileId, uploadName, "", map);
			if(StringUtil.isNotEmpty(inputObjNum)){   //只有火狐和谷歌浏览器才会加入并有这个参数
				reutrnStr = inputObjNum+"##"+reutrnStr;     //返回到前台（用于火狐和谷歌浏览器）时再解析
			}				
			if(StringUtil.isNotEmpty(ajaxType) && "obj".equals(ajaxType)&& StringUtil.isNotEmpty(reutrnStr)){
	...
	public void getFileById(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		if(bytes==null || bytes.length<1 ){
			String filePath = sysFile.getFilePath();
			String fullPath = StringUtil.trimSufffix(attachPath, File.separator) + File.separator + filePath.replace("/", File.separator);
			/*if("small".equals(type)){           //保有注销留获取压缩的图片
	...
	public void getFile(HttpServletRequest request, HttpServletResponse response, Long fileId) throws IOException {
		...
		if(saveType.contains("database")){
			if(sysFile.getFileBlob()==null||sysFile.getFileBlob().length<1){
				...
				outp.write(sysFile.getFileBlob());
			}
		}else{
	...
	public void saveFilePdf(MultipartHttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		//	long userId = ContextUtil.getCurrentUserId(); // 获取当前用户的id
			...
			Map<String, MultipartFile> files = request.getFileMap();
			Map map = new HashMap();
			String id = this.save(files, typeId, fileId, uploadName, "pdf", map);
			writer.print(id);
		} catch (Exception e) {
			logger.warn(e.getMessage());
			writer.print(-1);
	...
	//		long userId = ContextUtil.getCurrentUserId(); // 获取当前用户的id
			...
			Map<String, MultipartFile> files = request.getFileMap();
			Map map = new HashMap();
			String id = this.save(files, typeId, fileId, uploadName, "info", map);
			writer.print(id);
		} catch (Exception e) {
			logger.warn(e.getMessage());
			writer.print(-1);
	...
	public String save(Map<String, MultipartFile> files,long typeId,long fileId,String uploadName,String fileType,Map map) throws IOException{
		long userId = ContextUtil.getCurrentUserId(); // 获取当前用户的id
//		String ajaxType = (String)map.get("ajaxType");
//		String inputObjNum = (String)map.get("inputObjNum");;   //在OFFICE控件对象数组的的序号，用于返回前台时，把相应的值存放在相应的对象里
		SysUser appUser = null;
		String reutrnStr = "";
		if(userId>0){			
			appUser = sysUserService.getById(userId);
		...
		while (it.hasNext()) {
			...
			if(uploadName.equals(name)){
				...
				if(StringUtil.isEmpty(fileType)){
			    	extName=FileUtil.getFileExt(oriFileName);
			    }else{
			    	extName=fileType;
			    }
			    ...
			    // end 向数据库中添加数据
			    reutrnStr = Long.toString(fileId);
			}			
		}
		return reutrnStr;
	} 
