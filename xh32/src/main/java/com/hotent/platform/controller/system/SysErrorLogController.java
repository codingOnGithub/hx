...
public class SysErrorLogController extends BaseController {
	...
	public ModelAndView list(HttpServletRequest request,
		...
		List<SysErrorLog> list = sysErrorLogService.getAll(new QueryFilter(
				request, "sysErrorLogItem"));
	 ...
	 */
	@RequestMapping("del")
	@Action(description = "删除系统错误日志", execOrder = ActionExecOrder.BEFORE, detail = "删除系统附件"
			+ "<#list StringUtils.split(id,\",\") as item>"
			+ "<#assign entity=sysErrorLogService.getById(Long.valueOf(item))/>"
			+ "【${entity.hashcode}】" + "</#list>")
	...
	@ResponseBody
	public Object getErrorInfo(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		long id = RequestUtil.getLong(request, "id");
		SysErrorLog sysErrorLog = sysErrorLogService.getById(id);
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("sysErrorLog", sysErrorLog);
		return map;
	}
