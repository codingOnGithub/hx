		...
		try{
			...
			if(BeanUtils.isEmpty(tempId) || tempId==0){
				...
				if(it.hasNext()) {
					...
					fileExt = FileUtil.getFileExt(oriFileName);
					...
					if(new File(uploadFile).exists() && new File(compileFilePath).exists()){
						writeResultMessage(response.getWriter(),getText("controller.sysReport.fileAlready",oriFileName),ResultMessage.Fail);
						return ;
					}
			...
			if(e.getMessage()==null)
				writeResultMessage(response.getWriter(),getText("controller.save.fail"),ResultMessage.Fail);
			else
				writeResultMessage(response.getWriter(),e.getMessage(),ResultMessage.Fail);
		} finally {
			if("zip".equals(fileExt) && StringUtil.isNotEmpty(uploadFile))
				//删除压缩包
