...
public class SysUserController extends BaseController {
	...
	public void del(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			delByIds(lAryId);
			message = new ResultMessage(ResultMessage.Success, getText("record.deleted",getText("userUnder.user")));
			
		} catch (Exception e) {
			message = new ResultMessage(ResultMessage.Fail, getText("record.deleted.fail",getText("userUnder.user")));
		}
	...
	public ModelAndView getEditMv(HttpServletRequest request,ModelAndView mv){
		...
		if (userId != 0) {
			...
			if(orgId>0){//posid???
				...
				mv.addObject("userPosList", userPosList);
			}
		}
	...
	public void modifyPwd(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(StringUtil.isEmpty( newPassword)||StringUtil.isEmpty( primitivePassword)){
			writeResultMessage(response.getWriter(), getText("controller.sysUser.modifyPwd.pwdIsNOtEmpty"), ResultMessage.Fail);	
		}else if(!enPassword.equals(password)){
			writeResultMessage(response.getWriter(), getText("controller.sysUser.modifyPwd.pwdError"), ResultMessage.Fail);		
		}else if(primitivePassword.equals(newPassword)){
			writeResultMessage(response.getWriter(), getText("controller.sysUser.modifyPwd.pwdSame"), ResultMessage.Fail);
		}else{
			try {
				sysUserService.updPwd(userId, newPassword);
				writeResultMessage(response.getWriter(),getText("controller.sysUser.modifyPwd.pwdSuccess"), ResultMessage.Success);
				issuccess=true;
			} catch (Exception ex) {
				String str = MessageUtil.getMessage();
				if (StringUtil.isNotEmpty(str)) {
					ResultMessage resultMessage = new ResultMessage(ResultMessage.Fail,getText("controller.sysUser.modifyPwd.pwdFail")+":" + str);
	...
	public void resetPwd(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			writeResultMessage(response.getWriter(), getText("controller.sysUser.resetPwd.success"), ResultMessage.Success);
		} catch (Exception ex) {
			String str = MessageUtil.getMessage();
			if (StringUtil.isNotEmpty(str)) {
				ResultMessage resultMessage = new ResultMessage(ResultMessage.Fail,getText("controller.sysUser.resetPwd.fail")+":" + str);
				response.getWriter().print(resultMessage);
			} else {
				String message = ExceptionUtil.getExceptionMessage(ex);
				ResultMessage resultMessage = new ResultMessage(ResultMessage.Fail, message);
	...
	public void editStatus(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			writeResultMessage(response.getWriter(), getText("controller.sysUser.editStatus.success"), ResultMessage.Success);
		} catch (Exception ex) {
			String str = MessageUtil.getMessage();
			if (StringUtil.isNotEmpty(str)) {
				ResultMessage resultMessage = new ResultMessage(ResultMessage.Fail,getText("controller.sysUser.editStatus.fail")+":" + str);
				response.getWriter().print(resultMessage);
			} else {
				String message = ExceptionUtil.getExceptionMessage(ex);
				ResultMessage resultMessage = new ResultMessage(ResultMessage.Fail, message);
...
//		sysUserList.addAll(sysUserService.getUserByQuery(queryFilter));
	...
	public void syncLdap(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		SysUser sysUser=sysUserService.getById(userId);
		if(sysUser==null){
			message=new ResultMessage(ResultMessage.Fail, getText("controller.sysUser.syncLdap.getDataFromBD"));
			addMessage(message, request);
			response.sendRedirect(preUrl);
			return;
		}
		...
		if(ldapUser==null){
			message=new ResultMessage(ResultMessage.Fail, getText("controller.sysUser.syncLdap.getDataFromAD"));
			addMessage(message, request);
			response.sendRedirect(preUrl);
			return;
		}
		...
		message=new ResultMessage(ResultMessage.Success, getText("controller.sysUser.syncLdap.datasyncSuccess"));
		addMessage(message, request);
		response.sendRedirect(preUrl);
	}
	/**
	...
	public void syncUser(HttpServletRequest request, HttpServletResponse response) throws Exception {
		ResultMessage message=new ResultMessage(ResultMessage.Success, getText("controller.sysUser.syncLdap.datasyncSuccess"));
		PrintWriter writer=response.getWriter();
		boolean isAvaible=isLdapConnnectable();
		if(!isAvaible){
			message=new ResultMessage(ResultMessage.Fail, getText("controller.sysUser.syncUser.isAvaibleFail"));
		}
		else{
			try{
