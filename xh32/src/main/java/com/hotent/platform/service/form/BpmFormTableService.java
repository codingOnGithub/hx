...
public class BpmFormTableService extends BaseService<BpmFormTable> {
	...
	public void updExtTable(BpmFormTable bpmFormTable) {
	...
	public int upd(BpmFormTable table, int generate) throws Exception {
		...
		// 修改之前的字段列表
		List<BpmFormField> originFieldList = bpmFormFieldDao
			...
			// 未发布
				...
				if (table.getIsMain() == 0 && generate == 1) {
					...
					if (mainTable.getIsPublished().shortValue() == 0) {
						List<BpmFormField> fieldList = convertFields(fields,
								false);
						table.setFieldList(fieldList);
						createTable(table);
					}
	...
	private void createTable(BpmFormTable table) throws SQLException {
		...
		// 创建表的国际化资源
//		bpmFormLanguageService.createTableInternational(table);
	...
	public void delByTableId(Long tableId) {
		...
		// 删除和表相关的国际化资源记录
		bpmFormLanguageService.delByFormIdAndType(tableId.toString(),BpmFormLanguage.BPM_FORM_TYPE);
	...
	private void getTabInfo(BpmFormTable bpmFormTable, int need) {
		...
		List<BpmFormField> fieldList = getBpmFormFieldList(tableId, need);
		...
		for (BpmFormTable table : subTableList) {
			List<BpmFormField> subFieldList = getBpmFormFieldList(
	...
	private List<BpmFormField> getBpmFormFieldList(Long tableId, int need) {
		List<BpmFormField> fieldList = null;
		if (need == BpmFormTable.NEED_HIDE) {
			fieldList = bpmFormFieldDao.getByTableIdContainHidden(tableId);
		} else if ((need == BpmFormTable.NEED_DELETE)) {
	...
	public BpmFormTable getByDefId(Long defId) throws Exception {
		...
		if (bpmFormTable == null)
			return null;
		getTabInfo(bpmFormTable, BpmFormTable.NEED_HIDE);
		return bpmFormTable;
	}
	...
	public BpmFormTable getTableById(Long tableId) {
		return getByTableId(tableId, BpmFormTable.NEED_NORMAL);
	}
	...
	private void importUpdTable(BpmFormTable destFormTable,
		...
		if (hasData) {// 如果存在数据
			...
			try {
				...
				// 存在子表的问题
				BpmFormTable bpmFormTable = this
	...
	private void resetFormTableInfo(BpmFormTable bpmFormTable) {
		...
		dao.update(bpmFormTable);
		List<BpmFormField> fields = bpmFormFieldDao.getByTableId(bpmFormTable
				.getTableId());
		for (BpmFormField field : fields) {
			int isDeleted = field.getIsDeleted() == null ? BpmFormField.IS_DELETE_N
					: field.getIsDeleted();
