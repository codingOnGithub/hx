...
public class BpmFormDefService extends BaseService<BpmFormDef> {
	...
	private  String parseUseInfo(String mode, boolean flag ) {
		...
		if(flag){//导出只对人员处理
			if(BpmFormRights.TYPE_USER.equals(type)){
				...
				for(String tempId:idArr){
					sysUser  =(SysUser)sysUserDao.getById(Long.parseLong(tempId));
					accounts.append(sysUser.getAccount()).append(",");
				}
		...
		//导入
			...
			for(int i=0;i<fullnameArr.length;i++){
				...
				//用户
				if(BpmFormRights.TYPE_USER.equals(type)){
					SysUser sysUser  =(SysUser)sysUserDao.getById(Long.parseLong(idArr[i]));
				...
				//角色
					...
					if(BeanUtils.isEmpty(sysRoleList) ){	
						MsgUtil.addMsg(MsgUtil.ERROR, getText("service.bpmFormDef.parseUseInfo.roleName.notExist",args));	
						count++;
					}else if(sysRoleList.size() > 1){
						MsgUtil.addMsg(MsgUtil.ERROR, getText("service.bpmFormDef.parseUseInfo.roleName.moreOne",args));
						count++;
					}else{
				...
				//组织或组织负责人
					...
					if(BeanUtils.isEmpty(sysOrgList) ){
						...
						MsgUtil.addMsg(MsgUtil.ERROR, getText("service.bpmFormDef.parseUseInfo.orgName.moreOne",args));
						count++;
					}else{
				...
				//岗位
					...
					if(BeanUtils.isEmpty(positionList) ){
						...
						MsgUtil.addMsg(MsgUtil.ERROR, getText("service.bpmFormDef.parseUseInfo.posName.moreOne",args));
						count++;
					}else{
	...
	/**
	 * 保存或更新表单其他国际化资源
	 * @param bpmFormLanguages
	 */
//	public void handlerBpmFormLan(List<BpmFormLanguage> bpmFormLanguages,Long formId){
//		for(BpmFormLanguage bpmFormLanguage : bpmFormLanguages){
//			bpmFormLanguage.setFormid(formId);
//			if(bpmFormLanguage.isAdd()){
...
//	}
	...
	private void importBpmFormDef(BpmFormDef bpmFormDef) throws Exception {
		...
		dao.add(bpmFormDef);
		MsgUtil.addMsg(MsgUtil.SUCCESS, getText("service.bpmFormDef.import.bpmFormDef.isNew",new Object[]{bpmFormDef.getSubject()}));
	}
	...
	private void importBpmDataTemplate(BpmDataTemplate bpmTableTemplate,
		...
		if (BeanUtils.isNotEmpty(tableTemplate)) {
			...
			MsgUtil.addMsg(MsgUtil.WARN, getText("service.bpmFormDef.import.bpmDataTemplate.isExist",new Object[]{tableTemplate.getId()}));
			return;
		}
		bpmTableTemplate.setFormKey(formKey);
		bpmDataTemplateDao.add(bpmTableTemplate);
		MsgUtil.addMsg(MsgUtil.SUCCESS, getText("service.bpmFormDef.import.bpmDataTemplate.isNew",new Object[]{bpmTableTemplate.getName()}));
	}
