...
public class SealRightService extends BaseService<SealRight>
{
	...
	public void saveSealRight(Long sealId,String rightType,String rightIds,String rightNames,Long userId){
		...
		if("all".equals(rightType)){
			...
			sr.setRightId(0l);
			sr.setRightName(getText("service.sealRight.all"));
			sr.setCreateUser(userId);
			sr.setCreateTime(date);
			sealRightDao.add(sr);
		}else if("not".equals(rightType)){
            logger.info(getText("service.sealRight.no"));
	...
	public List<Map> getRightType(){
		...
		map = new HashMap();
		map.put("id", "not");
		map.put("name", getText("service.sealRight.none"));
		list.add(map);
		
		map = new HashMap();
		map.put("id", "user");
		map.put("name", getText("service.sealRight.user"));
		list.add(map);
		
		map = new HashMap();
		map.put("id", "role");
		map.put("name", getText("service.sealRight.role"));
		list.add(map);
		
		map = new HashMap();
		map.put("id", "org");
		map.put("name", getText("service.sealRight.org"));
		list.add(map);
		
		map = new HashMap();
		map.put("id", "all");
		map.put("name", getText("service.sealRight.all"));
		list.add(map);
