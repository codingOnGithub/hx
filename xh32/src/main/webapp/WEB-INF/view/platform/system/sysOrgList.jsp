...
<title><spr:message code="sysOrg.title"/><spr:message code="title.manage"/></title>
			...
			for (var i=0, l=childNodes.length; i<l; i++) {
				...
				if (node.isRoot == 1) {
					...
					if (node.ownUser == null || node.ownUser.length < 1) {
						node.orgName += $lang_system.sysOrg.notfilter;
					}
// 					setIcon(node);
				}
			}
	...
	function getMenu() {
		menu = $.ligerMenu({
			...
			items:<f:menu>
				[ 
				  {
					text : '<spr:message code="sysOrg.items.addOrg"/>',
					click : 'addNode',
					alias:'addOrg'
				}, {
					...
					text : '<spr:message code="sysOrg.items.sortOrg"/>',
					click : 'sortNode',
					alias:''
				}, {
					...
					text : '<spr:message code="sysOrg.items.refreshOrg"/>',
					click : 'refreshNode',
					alias:''
				},{
					text : '<spr:message code="sysOrg.items.addPos"/>',
					click : 'addPosNode',
					alias:''
				}
				]
	...
	function delNode() {
		...
		$.ligerDialog.confirm($lang_system.sysOrg.delConfirm, $lang.tip.msg, callback);
...
<body>
	...
	<div id="layout" style="bottom: 1; top: 1">
		<div position="left" title="<spr:message code="sysOrg.title"/><spr:message code="title.manage"/>" id="rogTree"
			style="height: 100%; width: 100% !important;">
			<div style="width: 100%;">
				<select id="demensionId" style="width: 99.8% !important;">
					<option value="0">---------<spr:message code="search.select.all"/>---------</option>
		...
		<div position="center" id="orgView" style="height: 100%;">
			<div class="l-layout-header"><spr:message code="sysOrg.title"/><spr:message code="title.detail"/></div>
