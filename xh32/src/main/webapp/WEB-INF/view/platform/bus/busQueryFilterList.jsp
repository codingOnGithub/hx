				...
				<c:choose>
					<c:when test="${!empty myFilterList && fn:length(myFilterList) >0 }">
						<c:forEach items="${myFilterList}" var="busQueryFilterItem">
							<tr>
								<td><a href="#" onclick="view(${busQueryFilterItem.id},'${busQueryFilterItem.filterKey}','${busQueryFilterItem.query}')"><c:if test="${busQueryFilterItem.isShare ==1}">【共享】</c:if>${busQueryFilterItem.filterName}</a>
