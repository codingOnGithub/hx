...
<%
	...
	String localeLang=request.getAttribute("localeLang").toString();
	// 在flex中保存，发布流程的请求地址
...
%>
  ...
  <head>
  	<title>
  		<spr:message code="bpmDefinition.design.title"/>--<c:choose>
  			<c:when test="${not empty bpmDefinition}">
  				${bpmDefinition.subject} --(<spr:message code="bpmDefinition.versionNo"/>${bpmDefinition.versionNo})
  			</c:when>
  			<c:otherwise>
  				${subject}
  			</c:otherwise>
  ...
  <style type="text/css" media="screen"> 
	...
	#flashContent { display:none; }
       ...
       <script type="text/javascript">
	       ...
	       swfobject.embedSWF("<%=basePath%>/media/swf/bpm/bpmeditor_<%=localeLang%>.swf", "flashContent", 
...
<noscript>
    <object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="100%" height="100%" id="bpmeditor">
        <param name="movie" value="<%=basePath%>/media/swf/bpm/bpmeditor_<%=localeLang%>.swf" />
        ...
        <!--[if !IE]>-->
        <object type="application/x-shockwave-flash" data="<%=basePath%>/media/swf/bpm/bpmeditor_<%=localeLang%>.swf" width="100%" height="80%">
            <param name="quality" value="high" />
            <param name="bgcolor" value="#ffffff" />
            <param name="allowScriptAccess" value="sameDomain" />
            <param name="allowFullScreen" value="true" />
