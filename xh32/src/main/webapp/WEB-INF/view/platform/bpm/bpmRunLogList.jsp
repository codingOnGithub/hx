...
<title><spr:message code="bpmRunLog.title"/><spr:message code="operator.manageList"/></title>
			...
			<div class="hide-panel">
				<div class="panel-top">
					<div class="tbar-title">
						<span class="tbar-label"><spr:message code="bpmRunLog.title"/><spr:message code="operator.manageList"/></span>
					</div>
					<div class="panel-toolbar">
						<div class="toolBar">
							...
							<c:if test="${runId>0}">
								<div class="group"><a class="link back" onclick="javascript:history.back()"><span></span><spr:message code="menu.button.back"/></a></div>
							</c:if>
						</div>	
					</div>
					<c:if test="${runId==0}">
						<div class="panel-search">
								<form id="searchForm" method="post" action="list.ht">
										<ul class="row">
											<li><span class="label"><spr:message code="bpmRunLog.processSubject"/>:</span><input type="text" name="Q_processSubject_SL"  class="inputText"  value="${param['Q_processSubject_SL']}"/></li>
											<li><span class="label"><spr:message code="bpmRunLog.username"/>:</span><input type="text" name="Q_username_SL"  class="inputText"  value="${param['Q_username_SL']}"/></li>
											<div class="row_date">
											<li><span class="label"><spr:message code="bpmRunLog.createtime"/> <spr:message code="search.from"/>:</span> <input  name="Q_begincreatetime_DL"  class="inputText date"  value="${param['Q_begincreatetime_DL']}"/></li>
											...
											<li><span class="label"><spr:message code="bpmRunLog.operatortype"/>:</span>
											<select name="Q_operatortype_S" class="select">
												...
												<option value="22" <c:if test="${param['Q_operatortype_S'] == 22}">selected</c:if>><spr:message code="bpmRunLog.operatortype.saveDraft"/></option>
												<option value="23" <c:if test="${param['Q_operatortype_S'] == 23}">selected</c:if>><spr:message code="bpmRunLog.operatortype.finishDivert"/></option>
												<option value="24" <c:if test="${param['Q_operatortype_S'] == 24}">selected</c:if>><spr:message code="bpmRunLog.operatortype.deleteDraft"/></option>
												</select></li>
										</ul>
								</form>
						</div>
				...
				<div class="panel-body">
					    ...
					    <display:table name="bpmRunLogList" id="bpmRunLogItem" requestURI="list.ht" sort="external" cellpadding="1" cellspacing="1" class="table-grid">
							...
							<display:column titleKey="bpmRunLog.processSubject"  style="width:180px;" sortable="true" sortName="processSubject">
								<c:choose>
							...
							<display:column property="username" titleKey="bpmRunLog.username" style="width:80px;" sortable="true" sortName="username"></display:column>
							...
							<display:column titleKey="bpmRunLog.operatortype" sortable="true" style="width:150px;" sortName="operatortype">
								<c:choose>
									...
									<c:when test="${bpmRunLogItem.operatortype eq 22}"><span class="green"><spr:message code="bpmRunLog.operatortype.saveDraft"/></span></c:when>
									<c:when test="${bpmRunLogItem.operatortype eq 23}"><span class="green"><spr:message code="bpmRunLog.operatortype.finishDivert"/></span></c:when>
									<c:when test="${bpmRunLogItem.operatortype eq 24}"><span class="green"><spr:message code="bpmRunLog.operatortype.deleteDraft"/></span></c:when>
							...
							<display:column titleKey="list.manage" media="html" style="width:180px">
								<a href="del.ht?id=${bpmRunLogItem.id}" class="link del"><spr:message code="menu.button.del"/></a>								
								<a href="get.ht?id=${bpmRunLogItem.id}" class="link detail"><spr:message code="menu.button.detail"/></a>
							</display:column>
						</display:table>
