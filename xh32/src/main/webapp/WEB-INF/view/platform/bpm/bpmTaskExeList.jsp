...
<title><spr:message code="bpmTaskExe.title"/><spr:message code="title.manageList"/></title>
	...
	<div class="panel">
		<div class="panel-top">
			<div class="tbar-title">
				<span class="tbar-label"><spr:message code="bpmTaskExe.title"/><spr:message code="title.manageList"/></span>
			</div>
			...
			<div class="panel-search">
				<form id="searchForm" method="post" action="list.ht">
					<ul class="row">
					<li>
						<span class="label"><spr:message code="processCenter.subject"/>:</span><input type="text" name="Q_subject_SUPL"  class="inputText"   value="${param['Q_subject_SUPL']}"/>
					</li>
					<li>
						<span class="label"><spr:message code="processCenter.processName"/>:</span><input type="text" name="Q_processName_SUPL"  class="inputText"   value="${param['Q_processName_SUPL']}"/>						
						<input type="hidden" id="assignId" name="Q_assigneeId_L"   value="${param['Q_assigneeId_L']}">
					</li>
					<li>
						<span class="label"><spr:message code="processCenter.assignName"/>:</span>
					...
					<li>
						<span class="label"><spr:message code="processCenter.assignType"/>:</span>
					...
					<div class="row_date">
					<li>	
						<span class="label"><spr:message code="processCenter.createtime"/>&nbsp;<spr:message code="search.from"/>:</span><input  name="Q_begincratetime_DL"  class="inputText datePicker" datetype="1" value="${param['Q_begincratetime_DL']}" />
					</li>
					<li>
						<span class="label"><spr:message code="search.to"/>: </span><input  name="Q_endcratetime_DG" class="inputText datePicker"  datetype="2"  value="${param['Q_endcratetime_DG']}"/>
					</li>
					</div>
					<li>
						<span class="label"><spr:message code="processCenter.curStatus"/>:</span>
							<select type="text" name="Q_assignType_S" value="${param['Q_assignType_S']}">
								<option value="" ><spr:message code="search.select.allSelect"/></option>
								<option value="1" <c:if test="${param['Q_assignType_S'] == 1}">selected</c:if>><spr:message code="search.select.allSelect"/></option>
		...
		<div class="panel-body">
		    ...
		    <display:table name="bpmTaskExeList" id="bpmTaskExeItem" requestURI="accordingMattersList.ht" cellpadding="1" cellspacing="1"  class="table-grid">
				...
				<display:column titleKey="processCenter.assignName" sortable="true" sortName="assigneeName">
					<a href="javascript:userDetail('${bpmTaskExeItem.assigneeId}');">${bpmTaskExeItem.assigneeName}</a>
				</display:column>
				<display:column  titleKey="processCenter.createtime" sortable="true" sortName="cratetime">
					<fmt:formatDate value="${bpmTaskExeItem.cratetime}" pattern="yyyy-MM-dd"/>
				</display:column>
				<display:column titleKey="processCenter.assignType" sortable="true" sortName="status">
					<c:choose>
				...
				<display:column titleKey="processCenter.pendingType" sortable="true" sortName="assignType">
					<c:choose>
						<c:when test="${bpmTaskExeItem.assignType==1}">
							<span class="green"><spr:message code="processCenter.pendingType.agent"/></span>
						</c:when>
				...
				<display:column titleKey="list.manage" media="html" style="width:50px">
					<c:if test="${bpmTaskExeItem.status==0}">
						...
						<a href="#" onclick="cancel(${bpmTaskExeItem.id})" title="${cancel_title}"  class="link cancel"><spr:message code="operator.cancel"/></a>
					</c:if>
				</display:column>
			</display:table>
