...
<head>
	...
	<title><spr:message code="title.edit"/> <spr:message code="agentSetting"/></title>
	...
	<script type="text/javascript">
		$(function() {
			...
			$("a.save").click(function() {
				...
				if(frm.valid()){
					...
					if(rtn.status){
						if(rtn.msg){
							$.ligerDialog.tipDialog($lang.tip.msg,$lang_bpm.agent.settingEdit.settingWarn,rtn.msg);
						}
						return;
					}
					form.submit();
		...
		*/
		function customValidate(){
			...
			case '0':
				...
				if(!agentId){
					rtn.status=-1;
					rtn.msg=$lang_bpm.agent.settingEdit.agentNotNull;
					return rtn;
				}
			...
			case '1':
				...
				if(!flows || flows.length<1){
					rtn.status = -1;
					rtn.msg=$lang_bpm.agent.settingEdit.partMustSelFlowDef;
					return rtn;
				}
				var agentSettingStatus = $("input[name='enabled']:checked").val();
				if(agentSettingStatus==1){
			...
			case '2':
				...
				if(!flowkey){
					rtn.status=-1;
					rtn.msg =$lang_bpm.agent.settingEdit.SelFlowDef;
					return rtn;
				}
				...
				if(conditions.length<1){
					rtn.status=-1;
					rtn.msg =$lang_bpm.agent.settingEdit.setAgentCondition;
					return rtn;
				}
		...
		*/
		function setCondition(){
			...
			if(flowkey=="") {
				$.ligerDialog.warn($lang_bpm.agent.settingEdit.selectFlowFirst ,$lang.tip.msg);
				return;
			}
	...
	<div class="panel-top">
		<div class="tbar-title">
		    <c:choose>
			    <c:when test="${agentSetting.id !=null}">
			        <span class="tbar-label"><spr:message code="title.update"/><spr:message code="agentSetting"/></span>
			    </c:when>
			    <c:otherwise>
			         <span class="tbar-label"><spr:message code="title.add"/><spr:message code="agentSetting"/></span>
			    </c:otherwise>			   
		    </c:choose>
		</div>
		<div class="panel-toolbar">
			<div class="toolBar">
				...
				<div class="group"><a class="link back" href="javascript:history.back(-1);"><span></span><spr:message code="menu.button.back"/></a></div>
			</div>
		</div>
	</div>
	...
	<div class="panel-body">
			...
			<table class="table-detail" cellpadding="0" cellspacing="0" border="0" type="main">
				<c:if test="${agentType==1}">  <!-- 管理入口时 -->
					<tr>
						<th width="20%"><spr:message code="agentSetting.authname"/>: </th>
						<td colspan="3">
							...
							<a href="#" class="current" onclick="selectAuth();"><span><spr:message code="agentSetting.authname.select"/></span></a>
						</td>
					</tr>				
				</c:if>
				
				<tr>
					<th width="20%"><spr:message code="agentSetting.agentType"/>: </th>
					<td colspan="3">
						<c:choose>
							<c:when test="${agentSetting.id!=null && agentSetting.id!=0 }">
								<c:choose>
									<c:when test="${agentSetting.authtype==0}">
										<spr:message code="agentSetting.authtype.general"/>
										
									</c:when>
									<c:when test="${agentSetting.authtype==2 }">
										<spr:message code="agentSetting.authtype.condition"/>
									</c:when>
									<c:otherwise>
										<spr:message code="agentSetting.authtype.partial"/>
									</c:otherwise>
								</c:choose>
								<input type="radio" name="authtype" value="${agentSetting.authtype}" checked="checked" style="display: none"/>
							</c:when>
							<c:otherwise>
								<label><input type="radio" value="0" name="authtype" <c:if test="${agentSetting.authtype==0}">checked="checked"</c:if> ><spr:message code="agentSetting.authtype.general"/></label>
								<label><input type="radio" value="1" name="authtype" <c:if test="${agentSetting==null or agentSetting.authtype==1}">checked="checked"</c:if> ><spr:message code="agentSetting.authtype.partial"/></label>
								<label><input type="radio" value="2" name="authtype" <c:if test="${agentSetting.authtype==2}">checked="checked"</c:if> ><spr:message code="agentSetting.authtype.condition"/></label>
							</c:otherwise>
						</c:choose>
					</td>
				</tr>
				<tr>
					<th width="20%"><spr:message code="agentSetting.startdate"/>: </th>
					...
					<th width="20%"><spr:message code="agentSetting.enddate"/>: </th>
				...
				<tr id="trAgent"  <c:if test="${agentSetting.authtype==2}"> style="display: none"</c:if> >
					<th width="20%"><spr:message code="agentSetting.agent"/>: </th>
					<td colspan="3">
						...
						<a href="#" class="current" onclick="selectAgent();"><span><spr:message code="agentSetting.agent.select"/></span></a>
					</td>
				</tr>
				<tr>
					<th><spr:message code="agentSetting.enabled"/>:</th>
					<td colspan="3">
						<input id="enabled_y" name="enabled" value="0" type="radio" <c:if test="${agentSetting.enabled!=1}" >checked="checked" </c:if> /> <label for="enabled_y"><spr:message code="agentSetting.enabled.false"/></label>
						<input id="enabled_n" name="enabled" value="1" type="radio" <c:if test="${agentSetting==null or agentSetting.enabled==1}" >checked="checked" </c:if> /> <label for="enabled_n"><spr:message code="agentSetting.enabled.true"/></label>
					</td>
				</tr>
				<tr id="trDefinition" <c:if test="${ agentSetting.authtype !=2 }">style="display:none;" </c:if> >
					<th width="20%"><spr:message code="agentSetting.flowname"/>: </th>
					<td colspan="3">
						...
						<a href="#" class="current" onclick="selectFlow();"><span><spr:message code="agentSetting.flowname.select"/></span></a>
						<a href="#" class="current" id="linkAgentCondition" onclick="setCondition();"><span><spr:message code="agentSetting.edit.setCondition"/></span></a>
					</td>
				</tr>
			</table>
	</div>
			...
			<div id="bpmAgent"  <c:if test="${agentSetting.authtype==0 or agentSetting.authtype==2}"> style="display: none"</c:if> >
				<div class="panel-top">
					...
					<div class="panel-toolbar">
						<div class="toolBar">
							<div class="group">
								<a class="link add" href="#" onclick="addFlow();"><span></span><spr:message code="agentSetting.edit.addFlow"/></a>
							</div>
						</div>
					</div>
				</div>
				<div class="panel-body" >		
					   	<table  class="table-grid table-list"  Id = "agentDef" cellpadding="1" cellspacing="1" style="width:100%" type="sub">
					   		<thead>
					   			<tr>
					    			<th><spr:message code="agentSetting.flowname"/></th>
					    			<th width="150px"><spr:message code="list.manage"/></th>
					    		</tr>
					    	</thead>
					    		...
					    		<c:when test="${fn:length(agentSetting.agentDefList)>0}">
					    			<c:forEach items="${agentSetting.agentDefList}" var="bpmAgentItem">
							    		<tr id="def_${bpmAgentItem.flowkey}" type="subdata">
							    			...
							    			<td>
							    				<a href="#" class="link del" onclick="singleDel(this);" class="link del"><spr:message code="menu.button.del"/></a>
					    		...
					    		<c:otherwise>
					    			<tr id="firstRow">
					    				<td colspan="2" align="center">
					    					<font color='red'><spr:message code="agentSetting.edit.noFlow"/></font>
					    				</td>
					    			</tr>
					    		</c:otherwise>
					    	</c:choose>
