...
<title><spr:message code="bpmFormLanguage.title"/><spr:message code="operator.manageList"/><</title>
	...
	function showResponse(data){
		...
		if(obj.isSuccess()){//成功
			$.ligerDialog.confirm($lang.operateTip.successContinue,$lang.tip.msg,function(rtn){
				if(!rtn){
					location.href=__ctx+'/platform/form/bpmFormTable/list.ht';
				}
				else{
	...
	<div class="panel">
		<div class="panel-top">
			<div class="tbar-title">
				<span class="tbar-label"><spr:message code="bpmFormLanguage.title"/><spr:message code="operator.manageList"/></span>
			</div>
			<div class="panel-toolbar">
				<div class="toolBar">
					...
					<div class="l-bar-separator"></div>
					<div class="group"><a class="link del"  action="del.ht?formid=${formid}"><span></span><spr:message code="menu.button.del"/></a></div>
					<div class="l-bar-separator"></div>
					<div class="group"><a class="link back" href="${ctx}/platform/form/bpmFormTable/list.ht"><span></span><spr:message code="menu.button.back"/></a></div>
				</div>	
			</div>
			<div class="panel-search">
				<form id="searchForm" method="post" action="list.ht">
					<div class="row">
						<span class="label"><spr:message code="bpmFormLanguage.resDesc"/>:</span><input type="text" name="Q_reskey_SL"  value="${param['Q_reskey_SL']}" class="inputText" />
					</div>
				</form>
			</div>
		</div>
		<div class="panel-body">
			...
			<table id="tableColumnItem" cellpadding="1" cellspacing="1" border="0" class="table-grid">
				<thead>
					...
					<th width="15%"><spr:message code="bpmFormLanguage.resDesc"/></th>
				...
				<tbody>
					<c:forEach items="${bpmFormFieldList}" var="bpmFormField" varStatus="obj">
						...
						<c:if test="${obj.count%2!='0'}">
							...
							<td>
								<input type="checkbox" class="pk" name="resKey" tableId="${bpmFormField.tableId}" value="${bpmFormField.fieldName}">
							</td>
							<td>${bpmFormField.fieldDesc}</td>
							<c:forEach items="${languages}" var="language">
								<td class="lan" name="${language.language}">
									${bpmFormLanguageMap[bpmFormField.fieldName][language.language]}
								</td>
							</c:forEach>
							<td style="line-height:21px;">
								<a href="del.ht?resKey=${bpmFormField.fieldName}&formId=${formid}" class="link del"><spr:message code="menu.button.del"/></a>
							</td>
						</tr>
					</c:forEach>
				</tbody>
