...
<title><spr:message code="bpmGangedSet.title"/></title>
...
<script type="text/javascript">
		...
		$("#bpmGangedSetItem").delegate("select[name='changeTypeSel']","change",function(){
			...
			if(selectOpt.val()=='8'){
				...
				if(!controltype||controltype!="11"){
					$.ligerDialog.warn($lang_bpm.bpmGangedSet.selectCascadeTip);
					me.val('1');
				}
				else{					
	...
	function getChangeType(){
		var html = ['<select name="changeTypeSel"><option value="1">'+$lang_bpm.bpmGangedSet.changeType.hide+'</option><option value="2">'+$lang_bpm.bpmGangedSet.changeType.noHide+'</option>'];
		html.push('<option value="3">'+$lang_bpm.bpmGangedSet.changeType.read+'</option><option value="4">'+$lang_bpm.bpmGangedSet.changeType.noRead+'</option>');
		html.push('<option value="5">'+$lang_bpm.bpmGangedSet.changeType.required+'</option><option value="6">'+$lang_bpm.bpmGangedSet.changeType.noRequired+'</option>');
		html.push('<option value="7">'+$lang_bpm.bpmGangedSet.changeType.blank+'</option>');
		html.push('<option value="8">'+$lang_bpm.bpmGangedSet.changeType.cascade+'</option>');
		html.push('</select><a class="link done" style="display:none;">'+$lang_bpm.bpmGangedSet.cascadeSet+'</a>');
		return html.join('');
	};
	...
	function initNodeSelect(t){
		...
		for(var i=0;i<t.length;i++){
			...
			if(!c.nodeId&&!c.nodeName&&c.setId){	
				...
				html.push(c.setType);
				if(c.setType==2){					
					html.push('">'+$lang_bpm.bpmGangedSet.allNode+'</option>');
					globalFormkey = c.formKey;	
				}
				if(c.setType==1){
					tag = true;
					html.push('">'+$lang_bpm.bpmGangedSet.startNode+'</option>');
				}
			}
			else{
				html.push('<option setId="');
...
<body>
	 <jsp:include page="incDefinitionHead.jsp">
   		<jsp:param value="<spr:message code='bpmGangedSet.title'/>" name="title"/>
	</jsp:include>
		...
		<div class="panel">
			<div class="panel-top">
				<div class="tbar-title">
					<span class="tbar-label"<spr:message code="bpmGangedSet.title"/></span>
				</div>
				<div class="panel-toolbar">
					<div class="toolBar">
						<div class="group"><a class="link save" href="javascript:;"><span></span><spr:message code="menu.button.save"/></a></div>
						<div class="l-bar-separator"></div>
						<div class="group"><a class="link add" href="javascript:;"><span></span><spr:message code="menu.button.add"/></a></div>
						<div class="l-bar-separator"></div>
						<div class="group"><a class="link clean" href="javascript:;"><span></span><spr:message code="menu.button.del"/></a></div>
					</div>	
				</div>
			</div>
					...
					<table id="bpmGangedSetItem" cellpadding="1" cellspacing="1" border="1" style="border-collapse:collapse;" class="table-grid">
						<thead>
							<tr>
								...
								<th><spr:message code="bpmGangedSet.nodename"/></th>
								<th><spr:message code="bpmGangedSet.selectField"/></th>
								<th><spr:message code="bpmGangedSet.selectVal"/></th>
								<th><spr:message code="bpmGangedSet.changeField"/></th>
								<th><spr:message code="bpmGangedSet.changeType"/></th>
							</tr>
						</thead>
						<tbody>
							<tr class="empty">
								<td colspan="6">
									<div class="loading-div"><spr:message code="loading"/></div>
								</td>
							</tr>
						</tbody>
					</table>
