...
<html>
	<head>
		...
		<title><spr:message code="bpmNodeButton.startProcess"/>--${bpmDefinition.subject} --<spr:message code="bpmDefinition.version.info"/>:${bpmDefinition.versionNo}</title>
		...
		<script type="text/javascript">
			...
			function startWorkFlow(){
				...
				if(isNeedSubmitConfirm){
					$.ligerDialog.confirm($lang_bpm.task.startFlowForm.confirm_msg_start,$lang.tip.confirm,function(rtn){
						if(rtn){
			...
			function submitForm(action,button){
				...
				if(isFormEmpty){
					$.ligerDialog.warn($lang_bpm.task.startFlowForm.warn_msg_form_require,$lang.tip.warn);
					return;
				}
				...
				if(isExtForm){//提交第三方表单时检查该表单的参数
					...
					if(!rtn){
						$.ligerDialog.warn($lang_bpm.task.startFlowForm.warn_msg_form_validate,$lang.tip.warn);
						return;
					}
					//var isDraft=$(button).hasClass("isDraft");
					//var rtn=CustomForm.validate();
					//if(!rtn&&!isDraft) return;
					
					if(button!="a.save"){
						//子表必填检查(兼容新旧版本)					
						rtn = SubtablePermission.subRequired();
						if(!rtn){	
							$.ligerDialog.warn($lang_bpm.task.startFlowForm.warn_msg_form_subtable_require,$lang.tip.warn);
							return;
						}
					}
					//获取自定义表单的数据
			...
			function showResponse(responseText){
				...
				if(obj.isSuccess()){
					var msg=(isStartFlow)?$lang_bpm.task.startFlowForm.success_msg_start:$lang_bpm.task.startFlowForm.success_msg_form_save;
					$.ligerDialog.success(msg,$lang.tip.success,function(){
						//添加后置事件处理
					...
					var msg=(isStartFlow)?$lang_bpm.task.startFlowForm.error_msg_start:$lang_bpm.task.startFlowForm.error_msg_form_save;
					$.ligerDialog.err($lang.tip.error,msg,obj.getMessage());
				}
			}
		...
		<form id="frmWorkFlow" method="post" >
			...
			<div class="panel">
					...
					<div style="padding:6px 8px 3px 12px;" class="noprint">
						<b><spr:message code="start.startFlowForm.form.resume"/>：</b>${bpmDefinition.descp}
					</div>
					...
					<div class="panel-body printForm" style="overflow: auto;">
							...
							<c:choose>
								<c:when test="${bpmDefinition.showFirstAssignee==1}">
									<div id="jumpDiv" class="noprint" style="display:none;" tipInfo="<spr:message code="start.startFlowForm.form.loading"/>"></div>
								</c:when>
							</c:choose>
							<c:choose>
								<c:when test="${isFormEmpty==true}">
									<div class="noForm"><spr:message code="start.startFlowForm.form.none"/></div>
								</c:when>
								<c:otherwise>
									<c:choose>
