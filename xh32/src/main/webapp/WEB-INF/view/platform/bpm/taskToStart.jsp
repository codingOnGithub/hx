...
<html>
	<head>
		...
		<title><spr:message code="task.toStart.title" arguments="${task.name}"/></title>
	...
	/* 	    border:1px solid red; */
		...
		function showTaskCommunication(conf){
			...
			isTaskEnd(function(){
				...
				var winArgs="dialogWidth=650px;dialogHeight=280px;help=0;status=0;scroll=0;center=1";
				url=url.getNewUrl();
		...
		function isTaskEnd(callBack){
			...
			$.post(url,params,function(responseText){
				...
				if(obj.isSuccess()){
					...
					$.ligerDialog.warn($lang_bpm.task.toStart.warn_msg_complete_or_terminate,$lang.tip.warn);
				}
			});
		}
		...
		function submitForm(action,button){
			...
			if(isEmptyForm){
				$.ligerDialog.error($lang_bpm.task.toStart.error_msg_no_form,$lang.tip.error);
				return;
			}
			...
			if(isExtForm){
				...
				if(!rtn){
					$.ligerDialog.warn($lang_bpm.task.toStart.warn_msg_form_validate,$lang.tip.warn);
					return;
				}
				...
				if(button!="#btnSave"){
					...
					if(!rtn){	
						$.ligerDialog.warn($lang_bpm.task.toStart.warn_msg_form_subtable_require,$lang.tip.warn);
						return;
					}
				}
		...
		function completeTask(){
			...
			if(nextPathId.length>0){
				...
				if(!v){
					$.ligerDialog.error($lang_bpm.task.toStart.warn_msg_syncpath_require,$lang.tip.warn);
					return;	
				}
			}
			var operatorObj=getByOperatorType();
			...
			if(isNeedSubmitConfirm){
				$.ligerDialog.confirm($lang_bpm.task.toStart.confirm_msg_operate,$lang.tip.confirm,function(rtn){
					if(rtn){
						submitForm(action,button);
					}
				});
		...
		function getByOperatorType(){
			...
			switch(operatorType){
				...
				case 1:
					obj.btnId="btnAgree";
					obj.msg=$lang_bpm.task.toStart.success_msg_operate;
					break;
				//反对
				case 2:
				...
				case 4:
					obj.btnId="btnReject";
					obj.msg=$lang_bpm.task.toStart.success_msg_reject;
					break;
				//驳回到发起人
				case 5:
					obj.btnId="btnRejectToStart";
					obj.msg=$lang_bpm.task.toStart.success_msg_reject_start;
					break;
				//保存表单
				case 8:
					obj.btnId="btnSave";
					obj.msg=$lang_bpm.task.toStart.success_msg_form_save
					break;
		...
		function getErrorByOperatorType(){
			...
			switch(operatorType){
				...
				//弃权
				case 3:
					rtn=$lang_bpm.task.toStart.error_msg_complete;
					break;
				//驳回
				case 4:
					rtn=$lang_bpm.task.toStart.error_msg_reject;
					break;
				//驳回到发起人
				case 5:
					rtn=$lang_bpm.task.toStart.error_msg_reject_start;
					break;
				//保存表单
				case 8:
					rtn=$lang_bpm.task.toStart.error_msg_form_save;
					break;
			}
			return rtn;
		}
		...
		function showResponse(responseText){
			...
			var obj=new com.hotent.form.ResultMessage(responseText);
			if(obj.isSuccess()){
				$.ligerDialog.success(operatorObj.msg,$lang.tip.msg,function(){
				...
				$.ligerDialog.err($lang.tip.error,title,obj.getMessage());
			}
		}
			...
			if($("#btnReject").length>0){
				$("#btnReject").click(function(){
					...
					if(voteContent.length==0||(content && content.trim()!='')){
						...
						$.ligerDialog.warn($lang_bpm.task.toStart.warn_msg_reject_start_opinion,$lang.tip.warn);
					}
				})
			}
			//驳回到发起人
		...
		function endTask(){
			...
			if(rtn){
				...
				$.post(url,params,function(responseText){
					var obj=new com.hotent.form.ResultMessage(responseText);
					if(!obj.isSuccess()){
						$.ligerDialog.err($lang.tip.error,$lang_bpm.task.toStart.error_msg_terminate,obj.getMessage());
						return;
					}
		...
		function changeAssignee(){
			if(${isCanAssignee}){
				...
				$.ligerDialog.warn($lang_bpm.task.toStart.warn_msg_curTask_notToDo,$lang.tip.warn);
			}
		};
		 ...
		 <div class="panel">
		            ...
		            <div id="topNavWrapper">
						<ul id="topNav">
					      ...
					      <div class="l-layout-header noprint" >
			            	<spr:message code="task.toStart.frmWorkFlow.header" arguments="${task.name},${bpmDefinition.subject},${bpmDefinition.versionNo}"/>
				          </div>
				          <c:choose>
				...
				<div class="panel-body">
					<c:choose>
						...
						<div id="jumpDiv" class="noprint" style="display:none;" tipInfo="<spr:message code='task.toStart.jump.loading'/>">
						</div>
						</c:when>
					</c:choose>
					<c:choose>
					...
					<div class="printForm" style="overflow: auto;">
							<c:choose>
								<c:when test="${isEmptyForm==true}">
									<div class="noForm"><spr:message code="task.toStart.form.noForm"/></div>
								</c:when>
								<c:otherwise>
									<c:choose>
					...
					<input type="hidden" id="nodeId" name="nodeId" value="${task.taskDefinitionKey}"/>  
					<%--驳回和撤销投票为再次提交 --%>
					<c:choose>
