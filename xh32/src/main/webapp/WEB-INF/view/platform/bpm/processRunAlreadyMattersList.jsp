...
<title><spr:message code="processCenter.alreadyMatters"/></title>
	...
	<div class="hide-panel">
		<div class="panel-top">
			<div class="tbar-title">
				<span class="tbar-label"><spr:message code="processCenter.alreadyMatters"/></span>
			</div>
			...
			<div class="panel-search">
					<form id="searchForm" method="post" action="alreadyMattersList.ht">
							...
							<div class="row_date">
							<li>
								<span class="label"><spr:message code="processCenter.createtime"/>&nbsp;<spr:message code="search.from"/>:</span> <input  name="Q_begincreatetime_DL"  class="inputText datePicker" datetype="1"  value="${param['Q_begincreatetime_DL']}" />
							</li>
							<li>
								<span class="label"><spr:message code="search.to"/>: </span><input  name="Q_endcreatetime_DG" class="inputText datePicker" datetype="2" value="${param['Q_endcreatetime_DG']}"/>
							</li>
							</div>
							<li>
								<span class="label"><spr:message code="processCenter.status"/>:</span><select name="Q_status_SN">
									...
									<option value="4" <c:if test="${param['Q_status_SN']==4}">selected</c:if>><spr:message code="processRun.status.recover"/></option>
									<option value="5" <c:if test="${param['Q_status_SN']==5}">selected</c:if>><spr:message code="processRun.status.reject"/></option>
									<option value="11" <c:if test="${param['Q_status_SN']==11}">selected</c:if>><spr:message code="processRun.status.restartTask"/></option>				
								</select>
							</li>
							</ul>
					</form>
		...
		<div class="panel-body">
			    ...
			    <display:table name="processRunList" id="processRunItem" requestURI="alreadyMattersList.ht" sort="external" cellpadding="1" cellspacing="1" class="table-grid">
					<display:column titleKey="list.orderNo" media="html" style="width:15px;">
					...
					<display:column  titleKey="processCenter.createtime" sortable="true" sortName="createtime" style="text-align:left">
						<fmt:formatDate value="${processRunItem.createtime}" pattern="yyyy-MM-dd HH:mm:ss"/>
					</display:column>
					<display:column titleKey="processCenter.duration" style="text-align:left" >					
						<c:choose>
					...
					<display:column titleKey="processCenter.status" sortable="true" sortName="status" style="text-align:left">
