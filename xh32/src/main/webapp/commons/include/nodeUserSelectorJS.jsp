...
function selectCmp(obj){
	...
	switch(assignType){
		...
		case "script":
			//showScript(cmpNames);
			showScriptDialog({callback:callback,cmpNames:cmpNames,defId:defId});
			break;
		//部门的上级类型部门的负责人
...
function showScriptDialog(conf){
	...
	url=__ctx + "/platform/bpm/bpmDefinition/showScript.ht?defId="+conf.defId+"&cmpNames="+conf.cmpNames.val();
	url=url.getNewUrl();
