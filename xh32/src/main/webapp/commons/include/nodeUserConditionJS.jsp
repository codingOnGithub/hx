...
function conditionDialog(tableId,edit)
{
	...
	var rtn=window.showModalDialog(url,'',winArgs);
	if(rtn){
		var conditionNodeTemplate = $('#conditionNodeTemplate').val();
		$.post(__ctx + '/platform/bpm/bpmDefinition/getUserSetByDefNodeId.ht', {defId:defId, nodeId:nodeId}, function(data){
			var trHtml = easyTemplate(conditionNodeTemplate,data).toString();
			$('#table_'+nodeId+'>tbody.data').html(trHtml);
			initConditionShow();
		});
	}
}
