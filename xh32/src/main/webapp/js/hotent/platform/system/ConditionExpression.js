...
com.hotent.platform.system.ConditionExpression = function(){
	...
	this.getResult=function(){
		if(this.hasError){
			return $lang_js.rule.notConformRule;
		}
	...
	this.validNode=function(node){
		...
		//条件节点，有子节点
			...
			if(nextNode){
				...
				if(nextNode.type=="3"){
					this.hasError=true;
					window.status=($lang_js.rule.cond_has_nextNode+" nextNode.type=='3'" +node.name +"," +node.id);
					return;
				}
				//下一个节点为条件节点
				else{
					...
					if(nextNode.children && nextNode.children.length>0)
					{
						window.status=(node.id +"," + node.name +$lang_js.rule.cond_has_nextNode+ " nextNode.children");
						this.hasError=true;
						return;
					}
				}
			...
			if(preNode){
				if(preNode.type=="3"){
					this.hasError=true;
					window.status=($lang_js.rule.cond_has_nextNode+" preNode.type=='3'");
					return;
				}
				//下一个节点为条件节点
				else{
					...
					if(preNode.children && preNode.children.length>0)
					{
						window.status=($lang_js.rule.cond_has_nextNode+" preNode.children");
						this.hasError=true;
						return;
					}
				}
		...
		else 
		{
			//没有下面的节点
			if(nextNode==null){
				window.status=($lang_js.rule.cond_no_nextNode+"," + childs + "," +node.name +","+ node.id);
				this.hasError=true;
				return;
			}
			//有下面的节点
				...
				if(nextNode.children==null && nextNode.type!='3'){
					window.status=($lang_js.rule.cond_node+"，nextNode.type!='3' childs:" + childs +"," + node.type +"," + node.name +"," + nextNode.type+","+nextNode.id);
					this.hasError=true;
					return;
				}
			}
			//没有前一个节点
			if(preNode==null){
				window.status=($lang_js.rule.cond_node+"，preNode==null" +node.id +"," +node.name);
				this.hasError=true;
				return;
			}
			//前面有的节点
				...
				if(preNode.children==null  && preNode.type!='3'){
					window.status=($lang_js.rule.cond_node+"，preNode.childs==null");
					this.hasError=true;
					return;
				}
			}
