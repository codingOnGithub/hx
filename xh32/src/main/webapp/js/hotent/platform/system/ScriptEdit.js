...
function save(formId){
	...
	for(var i=0;i<settingSpan.length;i++){
		...
		if(!dialog.val()){
			$.ligerDialog.warn(String.format($lang_system.conditionScript.edit.selectDialog,paraName),$lang.tip.warn);
			isReturnFalse = true;
			break;
		}else if(!param.val()){
			$.ligerDialog.warn(String.format($lang_system.conditionScript.edit.selectReturnValue,paraName),$lang.tip.warn);
			isReturnFalse = true;
			break;
		}
	};
