...
function save(){
	...
	if(!users){
		$.ligerDialog.warn($lang_bpm.nodeUser.condition.notUserSetting,$lang.tip.msg);
		return;
	}
	...
	$.post(__ctx+"/platform/bpm/bpmUserCondition/save.ht",param,function(response){
		...
		if(resultJson.result==1){
			$.ligerDialog.success($lang.save.success,$lang.tip.msg,function(rtn){
				//对话框
...
function previewUserSetting(defId){
	...
	if(!params) {
		alert($lang_bpm.nodeUser.condition.notUserSetting);
		return;
	}
