...
$(function() {
	...
	$("#add_custom").click(function(){
		if(dialog==null){
			dialog = $.ligerDialog.open({title:$lang_bpm.bpmCommonWsSet.addCustom,target:$("#custom_param_div"),width:400,height:250,buttons:
...
function updateCustomParam(item, dialog){
	...
	if(!name){
		$.ligerDialog.warn($lang.tip.warn,$lang_bpm.bpmCommonWsSet.nullArgument);
	}
...
function saveWebserviceSet(){
	...
	if(!alias){
		$.ligerDialog.warn($lang_bpm.bpmCommonWsSet.nullAlias);
		return;
	}
	...
	$.post(url,{setObj:setObj,customParams:customParams},function(d){
		var result = eval("("+d+")");
		if(result.result)
			$.ligerDialog.success(result.message,$lang.tip.msg,function(){
				this.location.href = "list.ht";
			});
		else
			$.ligerDialog.error(result.message,$lang.tip.errorMsg);
	});
};
...
var addMethodNode = function(t,depParse) {
	...
	var setting = {
		...
		callback : {
			beforeCheck : function(treeId, treeNode) {
				...
				if (treeNode.checked) {
					if (confirm(String.format($lang_bpm.bpmCommonWsSet.cancelBind, treeNode.name))) {
						var $div = $('#' + treeId + "-edit");
						if ($div.length < 1) {
							return;
						}						
	...
	$.ligerDialog.waitting($lang_bpm.bpmCommonWsSet.loading);
	...
	$.ajax({
		...
		success : function(result) {
			...
			var json = eval('(' + result + ')');
			if (!json.success) {
				$.ligerDialog.warn(json.msg,$lang.tip.warn);
				return false;
			}
			...
			$("[var=del]", operatorField).click(function(e) {
				$.ligerDialog.confirm(String.format($lang_bpm.bpmCommonWsSet.deleteMethod, json.method),$lang.confirm, function(rtn) {
							if (rtn) {
		...
		error : function(msg) {
			$.ligerDialog.closeWaitting();
			$.ligerDialog.error($lang.tip.errorMsg,$lang.tip.error);
			return;
		}
	});
};
...
function getByWsdlUrl() {
	...
	if ($.trim(wsdlUrl).length < 1) {
		$.ligerDialog.warn($lang_bpm.bpmCommonWsSet.fillinAdress,$lang.tip.warn);
		return;
	}
...
function loadWsTree() {
	...
	$.ligerDialog.waitting($lang_bpm.bpmCommonWsSet.searching);
	$.ajax({
				type : "POST",
				...
				error : function(msg) {
					$.ligerDialog.closeWaitting();
					$.ligerDialog.error($lang_bpm.bpmCommonWsSet.errorAdress,$lang.tip.error);
					return;
				}
			});
};
...
function zTreeOnDblClick(e, treeId, treeNode) {
	...
	if (treeNode.level == 0) {
		...
		if (wsTree == null) {
			$.ligerDialog.warn($lang_bpm.bpmCommonWsSet.searchMethod,$lang.tip.warn);
			return;
		}
		var treeNode = getSelectNode();
		if (treeNode && treeNode.level == 0) {
			$.ligerDialog.warn($lang_bpm.bpmCommonWsSet.selectMethod,$lang.tip.warn);
			return;
		}
...
// 关闭窗口
function closeWin() {
	$.ligerDialog.confirm($lang_bpm.bpmCommonWsSet.closeCurWindow,$lang.tip.confirm, function(rtn) {
		if (rtn) {
			window.close();
		}
	});
