...
$(function() {
	...
	$('#wsdlTxt').bind('focus', function(event) {
				var defaultVal = $lang_bpm.bpmDefinition.nodeSet.webServiceSet.inputAdd;
	...
	var setting = {
		...
		callback : {
			beforeCheck : function(treeId, treeNode) {
				...
				if (treeNode.checked) {
					if (confirm(String.format($lang_bpm.bpmNodeWebService.isDelCurBinding,treeNode.name))) {
						var $div = $('#' + treeId + "-edit");
						if ($div.length < 1) {
							return;
						}						
	...
	$.ajax({
		...
		success : function(result) {
			...
			$("[var=del]", operatorField).click(function(e) {
				$.ligerDialog.confirm(String.format($lang_bpm.bpmNodeWebService.isDelCurMethod,treeNode.name),$lang.tip.msg, function(rtn) {
							if (rtn) {
...
function getByWsdlUrl() {
	...
	if ($.trim(wsdlUrl).length < 1 || $('#wsdlTxt')[0].defaultValue == wsdlUrl) {
		$.ligerDialog.warn($lang_bpm.bpmDefinition.nodeSet.webServiceSet.notWebURL,$lang.tip.msg);
		return;
	}
...
function loadWsTree() {
	...
	$.ligerDialog.waitting($lang.save.waiting);
	$.ajax({
				type : "POST",
				...
				error : function(msg) {
					$.ligerDialog.closeWaitting();
					$.ligerDialog.error($lang_bpm.bpmDefinition.nodeSet.webServiceSet.webUrlError,$lang.tip.msg);
					return;
				}
			});
};
...
function zTreeOnDblClick(e, treeId, treeNode) {
	...
	if (treeNode.level == 0) {
		...
		if (wsTree == null) {
			$.ligerDialog.warn($lang_bpm.bpmDefinition.nodeSet.webServiceSet.webMethodWarn,$lang.tip.msg);
			return;
		}
		var treeNode = getSelectNode();
		if (treeNode && treeNode.level == 0) {
			$.ligerDialog.warn($lang_bpm.bpmDefinition.nodeSet.webServiceSet.webMethodcho,$lang.tip.msg);
			return;
		}
...
// 关闭窗口
function closeWin() {
	$.ligerDialog.confirm($lang_bpm.bpmDefinition.nodeSet.webServiceSet.closeWindow,$lang.tip.msg, function(rtn) {
				if (rtn) {
					window.close();
				}
			});
