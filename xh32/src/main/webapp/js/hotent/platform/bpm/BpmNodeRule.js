...
function updateContent(item,data){
	...
	//普通规则
	if(ruleType=='1'){
		labelSpan = $('<span class="label-span left"></span>').attr("ruleType",ruleType).text($lang_js.rule.common);
	...
	//脚本规则
		...
		labelSpan = $('<span class="label-span left"></span>').attr("ruleType",ruleType).text($lang_js.rule.script);
...
function rule2json(item){
	...
	//脚本规则
		...
		jobject.conDesc = $lang.operate.script;
	}
