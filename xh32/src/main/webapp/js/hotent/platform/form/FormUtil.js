		...
		if(!queryArr==false && queryArr.length>0){
			for(var i=0;i<queryArr.length;i++){
				...
				if(isMain=="true"){
					...
					preSelector = "div[tablename='"+queryArr[i].subTableName+"']" ;
				}
				
				//解析获取对应的数据
				$(":hidden[name$=':"+queryArr[i].id+"ID']",preSelector).each(function(){
					if($(this).val()!=""){
				...
				if(paramsValueString.indexOf(queryArr[i].name)<0){
					$("[name$=':"+queryArr[i].id+"']",preSelector).each(function(){
						var self = $(this) ;
						if(self.val()!=""){
							paramsValueString += queryArr[i].name + "=" + $(this).val() +"&" ;
							return false ;
							...
							if(!selfClass==false){
								if(selfClass.indexOf("validError")>=0){
									isReturn = true ;
									$.ligerDialog.warn($lang_js.form.formUtil.write+'--'+self.attr("lablename"),$lang.tip.msg);
									return false ;
								}
							}
						}
