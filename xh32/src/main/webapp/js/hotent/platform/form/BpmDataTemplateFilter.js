...
$(function(){
	...
	$("#sqlTip").qtip({
		content:{
			text:'<div>'+$lang_form.bpmDataTemplate.filter.tip1+
				',<p>例：String sql ="select ID,field from table where 1=1";<br/>' +
				'Object field = map.get("field");<br/>' +
				'if(field!=null)<br/>' +
				'&nbsp;&nbsp;sql += " and field like \'%"+field+"%\'";<br/>' +
				'return sql; </p>' +
				$lang_form.bpmDataTemplate.filter.tip2 +
				';<br/>'+$lang_form.bpmDataTemplate.filter.tip3+'</div>'
		}
	});
});
...
function updateContent(item,data){
	...
	//普通条件
	if(ruleType=='1'){
		labelSpan = $('<span class="label-span left"></span>').attr("ruleType",ruleType).text($lang_form.bpmDataTemplate.filter.commonCondition);
	...
	//脚本规则
		...
		labelSpan = $('<span class="label-span left"></span>').attr("ruleType",ruleType).text($lang_js.bpmDataTemplate.filter.scriptCondition);
		var judge = $('<div class="judge left margin-set"></div>').append($('<a name="script" href="###" onclick="editConditionScript(this)">'+$lang.operate.script+'</a>'));
		...
		if(ruleType=='3'){
			labelSpan = $('<span class="label-span left"></span>').attr("ruleType",ruleType).text($lang_js.bpmDataTemplate.filter.userAttribute);
		}
		else {
			labelSpan = $('<span class="label-span left"></span>').attr("ruleType",ruleType).text($lang_js.bpmDataTemplate.filter.orgAttribute);
		}
...
function save(){
	var condition = "",
		...
		rtn={};
	if(nameV == "" || keyV==""){
		alert($lang_form.bpmDataTemplate.filter.alert);
		return;
	}
	if(type ==2){
		editor.save();
