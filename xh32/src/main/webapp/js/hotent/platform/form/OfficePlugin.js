...
OfficePlugin={
		...
		init:function(){
			...
			for ( var i = 0; i < this.fileObjs.length; i++) {
				...
				var dwidth = widthStr.toLowerCase().replaceAll("px",""); 
				if(dwidth<700){
				...
				var dheight = heightStr.toLowerCase().replaceAll("px",""); 
				if(dheight<700){
				...
				if(right=="no"){
					...
					//加载office控件。
					officeObj.renderTo(divId,{fileId:fileId,doctype:doctype,myNum:myNum});
					//是否有office控件。
