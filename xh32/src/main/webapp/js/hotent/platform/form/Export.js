...
Export.initExportMenu = function (){
    var menu1 = { width: 150, 
    	items:[{ text: $lang_form.Export.exportAll,click:function(){
    				Export.exportExcel(this,0);
    			}},
        		{ text: $lang_form.Export.exportSelected,click:function(){
    				Export.exportExcel(this,1);
    			}},
        		{ text: $lang_form.Export.exportCurrent,click:function(){
    ...
    $("div.exportMenu").ligerMenuBar({
			items: [
     			  { text: '<a href="#" class="link export"><span></span>'+$lang.button.expor+'&nbsp;&nbsp;</a>', menu: menu1 }
                  ]});
...
Export.setExportField = function (obj,exportType){
	...
	if(exportDialog==null){
		...
		exportDialog=$.ligerDialog.open(
			{title:$lang_form.Export.exportField,
			target:$("#exportField"),
			width:400,height:250,
			buttons:
				[ {text :$lang.button.expor,onclick: function(){
...
Export.exportExcel =function (obj,exportType){
	...
	if(exportType == 2 && totalPage.val()<=0){
		alert($lang_form.Export.notRecord);
		return;
	}
