...
//			var subFileJsonList =obj.subFileJsonMap["subFileJsonList"];
		...
		add : function(table, beforeElement) {
			...
			if (frm) {
				self.openWin($lang.button.add, $(frm), table, beforeElement, function(form, table, beforeElement) {
					//校验数据
		...
		edit : function(table, row) {
			...
			form.data('row', row);
			this.openWin($lang.button.edit, form, table, null, function(form, table) {
				var v = form.data('validate');
				var rtn=v.valid();
				if(!rtn) return false;
		...
		getMenu : function(needEdit) {
			...
			if ((needEdit && this.menuWithEdit) || (!needEdit && this.menu)) {
				...
				var menu;
				var items = [ {
					text : $lang_js.customForm.insertBefore,
					click : function() {
					...
					text : $lang_js.customForm.insertAfter,
					click : function() {
						var row = $(menu.target).closest('[formType]');
						var table = row.closest('div[type=subtable]');
					...
					text : $lang_js.customForm.deleteRecord,
					click : function() {
					...
					text : $lang_js.customForm.moveUp,
					click : function() {
						var t = $(menu.target).closest('[formType]');
					...
					text : $lang_js.customForm.moveDown,
					click : function() {
						var t = $(menu.target).closest('[formType]');
	  ...
	  selectValue:function(select){
		    var html='';
 			var query=select.attr("selectquery"),queryJson;
			if(query!=undefined && query!=null ){
				query=query.replaceAll("'","\"");
				queryJson = JSON2.parse(query.replaceAll("'","\""));
			}else{
				queryJson = "";
				query = "";
			}
