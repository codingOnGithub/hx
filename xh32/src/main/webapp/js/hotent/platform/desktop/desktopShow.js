...
function addThisColumn(desktop) {	
	if ($(".active", $("#colshtml")).length == 0) {
		$.ligerDialog.warn($lang_system.desktopLayoutcol.selectOneColumn,$lang.tip.warn);
		return;
	}
...
function saveMycolumn(u){
	...
	$.post(__ctx + u,
			...
			function(reData){				
				...
				if(parStr.result==1)
					$.ligerDialog.success(parStr.message,function(){location.reload();},$lang.tip.msg);
				else
					$.ligerDialog.warn(parStr.message,function(){location.reload();},$lang.tip.warn);
			});
};
