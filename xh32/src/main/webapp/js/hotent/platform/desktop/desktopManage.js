var DesktopManage=function(){
	...
	this.addColumn=function(){
		...
		if(self.thisDiv==null){
			$.ligerDialog.error($lang_system.desktopLayoutcol.selectOneColumn,$lang.tip.msg);
			return;
		}
	...
	this.saveLayout=function(){
		...
		data+="]";
		if(data=="[]"){
			$.ligerDialog.error($lang_system.desktopLayoutcol.setNoColumns,$lang.tip.msg);
			return;
		}
		$.post(url,{"layoutId":layoutId,"data":data},function(reData){				
							var parStr=$.parseJSON(reData);				
							if(parStr.result==1)
								$.ligerDialog.success( parStr.message,$lang.tip.msg,function(){
									window.location.reload();
								});
							else
								$.ligerDialog.error(parStr.message, $lang.tip.msg,function(){
									window.location.reload();
								});
						});
	};
...
function getMore(url){
	$.post(__ctx+"/platform/console/getResourceNode.ht?columnUrl="+url,function(data){
		if(data==null){
			$.ligerDialog.error($lang_system.desktopLayoutcol.misconfigured,$lang.tip.msg);
		}else{
