            ...
            $.post( 
                ...
                function (jsonStr) { 
                    ...
                    if(columnUrl==null||columnUrl.trim()==''){
                        ...
                        +'<span id=more"'+jsonData[i].columnId+'" style="display:none"><a href="'+__ctx+columnUrl+'" class="more">'+$lang.operate.more+'</a>'+columnUrl+'</span>'
                        +'</div><div class="widget-content"><ul>';
                        htd = '</ul></div></li>'; 
                    }else{
                        htt = '<li  class="widget color-blue" col="'+jsonData[i].col+'" sn="'+jsonData[i].sn+'"  id="'+jsonData[i].columnId+'"><div class="widget-head"><h3>'+jsonData[i].columnName+'</h3>'
                        +'<span id=more"'+jsonData[i].columnId+'"><a href="'+__ctx+columnUrl+'" class="more">'+$lang.operate.more+'</a></span>'
                        +'</div><div class="widget-content"><ul>';
                        htd = '</ul></div></li>'; 
                    }
                    var items = "";  
...
com.hotent.platform.desktop.saveCol= function(layoutID,filename,layoutCols){
		...
		$.ajax({
			...
			success : function(result) {
				...
				if (msg.isSuccess()) {
					ref = true;
					$.ligerDialog.success($lang_system.desktopLayout.saveLayout.success,$lang.tip.msg);
				} else {
					$.ligerDialog.success($lang_system.desktopLayout.saveLayout.fail,$lang.tip.msg);
					ref = false;
				}
			}
		});
