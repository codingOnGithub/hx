...
.fieldSelect{width:90px !important;}
.fieldCheck{width:10px !important;}
</style>
</head>
<body>
	<div id="inputPanel">
	...
	<#list data as obj>
		...
		<tr class="fieldTr">
			<th>
			<#if (obj_index == 0)>主表参数</#if>
			</th>
			<td>
				<input type="checkbox" class="fieldCheck" />&nbsp;
				<!-- 主表字段 -->
				<input type="text" class="mainField" style="width:60px" value="\${obj.name}" readonly id="\${obj.id}"/>&nbsp;
				<select class="fieldSelect hidden">
					<option></option>
				</select>
			</td>
		</tr>
		<#else>
			<#list obj.items as sub>
			<tr class="fieldTr">
				<th><#if (sub_index == 0)>\${obj.name}参数</#if><span id="subTableName" class="hidden">\${obj.id}</span></th>
				<td>
					<input type="checkbox" class="fieldCheck" />&nbsp;
		...
		function bindData(dialogStr) {
			...
			for(var i=0;i<dialogQueryArr.length;i++){
				...
				$(":text#"+dialogQueryArr[i].id,".fieldTr").each(function(){
					if((isMain=="true" ^ $(this).attr("class")=="mainField")!=0) return ;
					//字段isMain和属性class==mainfield同时为true或者false时
					var subTableName = $(this).closest('tr').find('#subTableName:hidden').text();
					//如果子表tablename不同，则不处理
					if(subTableName != dialogQueryArr[i].subTableName) return ;
		...
		dialog.onok = function() {
			...
			$(':checkbox:checked','table .fieldTr').each(function(){
				...
				if($(this).siblings(':text').attr('class')!='mainField'){
					...
					var subTableName = $(this).closest('tr').find('#subTableName:hidden');
					queryObj.subTableName = subTableName.text();
				}
