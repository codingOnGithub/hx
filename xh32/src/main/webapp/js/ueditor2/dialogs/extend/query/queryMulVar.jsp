...
<script type="text/javascript">
			...
			for(var i=0;i<dialogQueryArr.length;i++){
				...
				$(":text#"+dialogQueryArr[i].condition,".fieldTr").each(function(){
					...
					if(dialogQueryArr[i].initValue!=''){
						...
						$(this).siblings('div.fielddiv').children(':text').val("--"+$lang_selector.query.presetvalue+"--");
					}
				});
			}
		}
	...
	dialog.onok = function() {
		...
		if (!bindingKey) {
			alert($lang_selector.query.select_key);
			return false;
		};
		if (!bindingValue) {
			alert($lang_selector.query.select_value);
			return false;
		};
		...
		$(':checkbox:checked','table .fieldTr').each(function(){
			...
			if(initvalue=="--"+$lang_selector.query.presetvalue+"--"){
				initvalue="";
			}
	...
	function initFieldsDiv(data) {
		...
		//生成“绑定参数”行
		if(item){
			opt = '<optgroup label="'+$lang_selector.query.main_field+'"></optgroup>';
			selectOpt.push(opt);
		...
		if (!parentTableClass || parentTableClass.length > 0) {
			//若为空，表示主表，则隐藏子表字段
			opt = '<optgroup label="'+$lang_selector.query.sub_field+'"></optgroup>';
			selectOpt.push(opt);
		...
		var items = [ {
			name : $lang_selector.query.value,
			id : 'value'
		}, {
			name : $lang_selector.query.key,
			id : 'key'
		} ];
		data.items = items;
	...
	function showSearch(obj,type){
		if(type){
			if(obj.value=="")
				obj.value="--"+$lang_selector.query.presetvalue+"--";
		}else{
			if(obj.value=="--"+$lang_selector.query.presetvalue+"--")
				obj.value="";
		}
	}
		...
		<tr class="fieldTr">
			<td >
				...
				<select class="fieldSelect" style="width:30%;margin-right: 5px;">
					<option value="-1" controlType="-1" >--<spr:message code="search.select.pleaseSelect"/>--</option>
				</select>
				<input type="text" class="fieldValue" style="width:25%" value="--<spr:message code='presetvalue'/>--" onfocus="if (value==defaultValue)value=''" onblur="if(!value)value=defaultValue"/>
				</div>
			</td>
		</tr>
	</#list>
