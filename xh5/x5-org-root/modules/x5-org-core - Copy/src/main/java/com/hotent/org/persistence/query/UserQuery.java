...
//        } 
    ...
    public QueryFilter getQueryFilter(){
    	...
    	FieldLogic fieldLogic = new DefaultFieldLogic();
    	for(Criteria c:this.oredCriteria){
    		if(c.isValid()){
    			FieldLogic logic = new DefaultFieldLogic(FieldRelation.OR);
        		fieldLogic.getWhereClauses().add(logic);
    			for(final Criterion criterion:c.criteria){
    				if(criterion.noValue){
    					logic.getWhereClauses().add(new WhereClause() {
							@Override
    					...
    					logic.getWhereClauses().add(new WhereClause() {
							@Override
							public String getSql() {
								return criterion.condition + " " + "#{" + criterion.property + "}";
								
							}
						});
    					//TODO
//    					filter.getParams().put(", value);
    				}else if(criterion.betweenValue){
    					logic.getWhereClauses().add(new WhereClause() {
							@Override
							public String getSql() {
								return criterion.condition + " " + criterion.value + " and " +criterion.secondValue;
							}
						});
    					...
    					logic.getWhereClauses().add(new WhereClause() {
							@Override
							public String getSql() {
								String value = "("+StringUtils.join((Collection) criterion.value, ",")+")";
