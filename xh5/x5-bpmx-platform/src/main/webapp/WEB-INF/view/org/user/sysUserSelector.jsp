...
<title><spr:message code="sysUser.selector.title"/></title>
...
<div  class="panel-top">
	<div class="panel-search">
		<form id="searchForm" method="post" action="${ctx}/platform/system/sysUser/selector.ht" >
			<ul class="row">
				...
				<li><span class="label" ><spr:message code="sysUser.fullname"/>:</span><input size="14" type="text" name="Q_fullname_SL"  class="inputText" style="width:60%;" value="${param['Q_fullname_SL']}"/>
				&nbsp;<a href="#" onclick="$('#searchForm').submit()" class='button'><span><spr:message code="menu.button.search"/></span></a></li>
			</ul>
		</form>
	</div>
</div>
	...
	<display:table  name="sysUserList" id="sysUserItem" requestURI="selector.ht" sort="external" cellpadding="1" cellspacing="1" export="false"  class="table-grid">
		...
		<display:column  style="width:60px;" property="fullname" titleKey="sysUser.fullname" sortable="true" sortName="fullname"></display:column>
		<display:column  titleKey="sysUser.dialog.orgName" sortable="false" >
			<c:choose>
				<c:when test="${sysUserItem.orgName==''}"><span class="red"><spr:message code="sysUser.dialog.notSeting"/></span></c:when>
