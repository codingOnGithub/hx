...
<head>
	...
	<title><spr:message code="sysUser.select"/> </title>
		...
		*/
		function load_Pos_Tree(){
			...
			var setting = {
		    		...
		    		callback: {
						...
						var p="?isSingle=${isSingle}&searchBy=<%=SysUser.SEARCH_BY_POS%>&posId=" + treeNode.posId;
							$("#userListFrame").attr("src", url + p);
							setCenterTitle($lang_system.sysOrg.select.selByPos+":" + treeNode.posName);
						}
					}
				};
				var url="${ctx}/platform/system/position/getAll.ht";
		...
		function handOrgClick(event, treeId, treeNode){
			...
			if(rtn){
				...
				$("#userListFrame").attr("src",url+p);
				setCenterTitle($lang_system.sysOrg.select.selByOrg+":"+treeNode.orgName);
			}
		}
		...
		function handRoleClick(event, treeId, treeNode){
			...
			if(rtn){
				...
				$("#userListFrame").attr("src", url + p);
				setCenterTitle($lang_system.sysOrg.select.selByRole+":" + treeNode.roleName);
			}
				...
				var setting = {
			    		...
			    		callback: {
							onClick: function(event, treeId, treeNode){
								...
								$("#userListFrame").attr("src",url+p);
								setCenterTitle($lang_system.sysOrg.select.selByRole+":"+treeNode.orgName);
							}
						}
			    };
...
<body>
	<div id="defLayout">
		<div id="leftMemu" position="left" title="<spr:message code='sysUser.dialog.title'/>" style="overflow: auto; float: left;width: 100%;height:94%;">
			<div title="<spr:message code='sysOrg.organization'/><input type='checkbox' id='chkOrg'><label for='chkOrg'><spr:message code='menu.button.choose'/></label>" style="overflow: hidden;">
				<table border="0" width="100%" class="table-detail">
					<tr >
						<td width="30%" nowrap="nowrap"><span class="label"><spr:message code="sysUser.dialog.dimension"/>:</span>
						</td>
						<td style="width:70%;">
            ...
            <%--
			<div title="岗位<input type='checkbox' id='chkPosition'><label for='chkPosition'></label>" style="overflow: hidden;">
				<div id="SEARCH_BY_POS" class='ztree'></div>
			</div>
			 --%>
           	<div title="<spr:message code='sysUser.pos'/><input type='checkbox' id='chkPosition'><label for='chkPosition'><spr:message code='menu.button.choose'/></label>" style="overflow: hidden;">
				<table border="0" width="100%" class="table-detail">
					<tr >
						<td width="30%" nowrap="nowrap"><span class="label"><spr:message code="sysUser.dialog.dimension"/>:</span>
						</td>
			...
			<div title="<spr:message code='sysUser.role'/><input type='checkbox' id='chkRole'><label for='chkRole'><spr:message code='menu.button.choose'/></label>" style="overflow: hidden;">
				<div class="tree-title" style="width: 100%;">
					<div class="panel-detail">
						<table border="0" width="100%" class="table-detail">
							<tr>
								<td width="30%" nowrap="nowrap">
									<span class="label"style="width:60px;"><spr:message code="sysUser.dialog.system"/>:</span>
								</td>
								<td colspan="2">
							...
							<tr>
								<td>
									<span class="label"style="width:60px;"><spr:message code="sysUser.role"/>:</span>
								</td>
								<td>
		...
		<div position="center" style="height:100%;">
			<div id="centerTitle" class="l-layout-header"><spr:message code="sysUser.dialog.allUser"/></div>
		...
		<div position="right" title="<span><a onclick='javascript:dellAll();' class='link del'><spr:message code='menu.button.empty'/></a><input type='text' class='quick-find' title='<spr:message code='menu.button.search'/>' style='width:60px;' /></span>"
		...
		<div position="bottom"  class="bottom" >
			<a href="#" class="button"  onclick="selectUser()" ><span class="icon ok"></span><span ><spr:message code="menu.button.choose"/></span></a>
