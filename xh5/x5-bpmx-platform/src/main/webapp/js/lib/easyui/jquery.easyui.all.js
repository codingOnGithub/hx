...
//					selectTab(container, getLiIndex(li));
					...
					if (popts.collapsible){
						...
						if(opts.onBeforeSelect.call(container, e, li.find('span.tabs-title').html(), getLiIndex(li)))
...
//		}
	...
	$.fn.tabs.defaults = {
		...
		onBeforeSelect:function(e,title,index){return true;}
	};
})(jQuery);
/**
				...
				$(this).find('tr').each(function(){
					...
					$(this).find('th').each(function(){
						...
						var col = $.extend({}, $.parser.parseOptions(this, [
    						'field','align','halign','order','sortName',
...
//						'<div class="datagrid-resize-proxy"></div>' +
		...
		function updateGridPanel(e,force){
			var opts = $.data(target, 'datagrid').options;
			if (opts.fit == true || force){
				setSize(target);
				setTimeout(function(){
		...
		var panelParent=panel.panel('panel').parent().eq(0);
		if(panelParent.is('body')){
			$(window).bind('resize',updateGridPanel);
		}
