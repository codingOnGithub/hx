...
;(function($){
	...
	$.LinkDivList = function($div,options){
		...
		function addComputing(item){
			...
			if(!select){
				select = $('<select class="computing-select"><option value="and">'+$lang.operateType.and+'</option><option value="or">'+$lang.operateType.or+'</option></span>').val(computing);
			}

			select.css({top:top-15,left:left+width+12,width:50});
			item.append(select);
		};
		...
		function assemble_Div(){
			var selected = $("div.con-item-selected").toggleClass("con-item-selected"),				
				branch = $('<div class="con-item con-branch"><span class="label-span">'+$lang_bpm.ruleSetting.groupRule+'</span></div>');
		...
		function initData2Div(ary,div){
			for(var i=0,c;c=ary[i++];){
				var itemDiv;
				if(c.branch){
					itemDiv = $('<div class="con-item con-branch"><span class="label-span">'+$lang_bpm.ruleSetting.groupRule+'</span></div>');
					div.append(itemDiv);
