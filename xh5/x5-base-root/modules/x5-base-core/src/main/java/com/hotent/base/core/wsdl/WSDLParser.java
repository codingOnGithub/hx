...
public class WSDLParser {
	 ...
	 * @return
	 */
	private boolean containOperation(ServiceInfo serviceInfo,
	 ...
	 * @return
	 */
	private boolean processComplex(ParameterInfo parameterInfo,
			String typeName, String partName, String attrName, Boolean isList) {
		if (typeName == null)
		...
		// 处理复杂类型
		XmlSchemaParticle xmlSchemaParticle = ((XmlSchemaComplexType) xmlSchemaType)
				.getParticle();
		...
		for (int j = 0; j < count; j++) {
			XmlSchemaSequenceMember xmlSchemaObject = xmlSchemaObjectCollection
					.get(j);
			if (xmlSchemaObject instanceof XmlSchemaElement) {
				...
				if (xmlSType instanceof XmlSchemaComplexType) {
					...
					complexType.setParentComplext(parameterInfo
							.getParentComplext());
					if (StringUtils.isNotEmpty(xmlSTypeName)) {
						if (parameterInfo.getParentComplext().get(
								xmlSType.getName()) != null)
							break; // 防死循环
						processComplex(complexType, xmlSType.getName(),
								partName, elementName, max > 1);
					} else { // 如果复杂类型中没有的直接扩展的话需再次处理
						complexType.getParentComplext().put(elementName,
								elementName);
					...
					processComplex(complexType, type, partName, elementName,
							false);
					...
					complexType.getComplextParams()
							.put(elementName, simpleType);
				}
			}
		}
	...
	private void processComplexType(XmlSchemaType type) {
		...
		XmlSchemaContentModel xmlSchemaContentModel = xmlSchemaComplexType
				.getContentModel();
		XmlSchemaParticle xmlSchemaParticle = xmlSchemaComplexType
				.getParticle(); // 取定义type
		if (xmlSchemaParticle == null && xmlSchemaContentModel != null) { // 若定义的参数包含扩展的属性
			XmlSchemaContent xmlSchemaContent = xmlSchemaContentModel
					.getContent();
			if (xmlSchemaContent instanceof XmlSchemaComplexContentExtension) {
				...
				XmlSchemaParticle xmlSchemaParticleExtion = xmlSchemaComplexContentExtension
						.getParticle();
		...
		List<XmlSchemaSequenceMember> xmlSchemaObjectCollection = xmlSchemaSequence
				.getItems(); // 取type中Sequence
		int count = xmlSchemaObjectCollection.size();
		for (int i = 0; i < count; i++) {
			...
			if (xmlSchemaType != null
					&& xmlSchemaType instanceof XmlSchemaComplexType) {
	...
	private void processImport(Definition defintion) throws WSDLException {
		...
		while (keys.hasNext()) {
			...
			Vector<ImportImpl> importImpls = (Vector<ImportImpl>) impMap
					.get(key);// 上面key对应的value
	...
	private void processInputParam(OperationInfo operationInfo, Input input) {
		List<ParameterInfo> inputParams = (List<ParameterInfo>) operationInfo
				.getInputParams();
	 ...
	 * @param operation
	 */
	private void processOperation(ServiceInfo serviceInfo,
		...
		if (extensions != null) {
			for (int i = 0; i < extensions.size(); i++) {
				ExtensibilityElement extElement = (ExtensibilityElement) extensions
						.get(i);
	...
	private void processOutputParam(OperationInfo operationInfo, Output output) {
		if (output == null)
			return;
		List<ParameterInfo> outputParams = (List<ParameterInfo>) operationInfo
				.getOutputParams();
	 ...
	 * @param params
	 */
	private void processParam(OperationInfo operationInfo, Map<?, ?> partMap,
		...
		for (Iterator<?> iterator = parts.iterator(); iterator.hasNext();) {
			...
			if (processComplex(parameterInfo, typeName, operationName, null,
					false)) {
	...
	private void processService(ServiceImpl service, String wsdlUrl) {
		...
		for (Iterator<?> iterator = ports.iterator(); iterator.hasNext();) { // 遍历文档中的服务
			...
			for (Iterator<?> iterator2 = operations.iterator(); iterator2
					.hasNext();) { // 遍历服务中的函数
	...
	private void processXmlSchema(XmlSchema xmlSchema) {
		Map<QName, XmlSchemaType> smlSchemaObjectTable = xmlSchema
				.getSchemaTypes();
		List<XmlSchemaObject> xmlSchemaObjs = xmlSchema.getItems();
		Set<QName> smlSchemaKeys = smlSchemaObjectTable.keySet();
		for (Iterator<QName> otheriterator = smlSchemaKeys.iterator(); otheriterator
				.hasNext();) {
