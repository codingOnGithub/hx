...
public class JsonPluginsUtil {
	...
	private static final Logger logger = Logger
			.getLogger(JsonPluginsUtil.class);
	...
	public static <T> T jsonToBean(String jsonString, Class<T> beanCalss) {
		...
		// new JsonTimestampMorpher());
		JSONUtils.getMorpherRegistry().registerMorpher(
				new JsonDateMorpher(DATE_FORMAT));
	...
	private static JsonConfig configJson(String[] excludes) {
		...
		jsonConfig.registerJsonValueProcessor(Date.class,
				new JsonPluginsUtilDateValueProcessor());
	...
	public static Map<String, Object> jsonToMap(String jsonString, boolean next) {
		JSONObject jsonObject = JSONObject.fromObject(jsonString);
		...
		while (keyIter.hasNext()) {
			...
			logger.debug("Key:" + key + ",type:" + jsonObject.get(key));

			Object tempValue = jsonObject.get(key);

			// 检查是否为集合
			if (AssertValue.isNotNull(tempValue)
					&& AssertValue.isNotNullAndNotEmpty(tempValue.toString())) {
				if (tempValue.toString().startsWith("[") && next) {
					logger.debug("Key" + key + "为集合，转换为集合操作。");
					String[] arrayValue = JsonPluginsUtil
							.jsonToStringArray(tempValue.toString());
					valueMap.put(key, arrayValue);
				} else {
